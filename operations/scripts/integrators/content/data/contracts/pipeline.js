...
function loadViewModel() {
  if (cachedViewModel) return cachedViewModel;
  const source = fs.readFileSync(VIEW_MODEL_SOURCE_PATH, "utf8");
  const exportNames = [...source.matchAll(/export const (\w+)\s*=/g)].map((m) => m[1]);
  const transformed = `${source.replace(/export const /g, "const ")}\nmodule.exports = { ${exportNames.join(", ")} };`;
  const context = { module: { exports: {} }, exports: {} };
  vm.runInNewContext(transformed, context, { filename: VIEW_MODEL_SOURCE_PATH });
  cachedViewModel = context.module.exports;
  return cachedViewModel;
}

function buildCanonicalPagePayload(payload) {
  const vmExports = loadViewModel();
  const historicalCategoriesRaw = vmExports.getHistoricalCategories(payload);
  const historicalCategories = historicalCategoriesRaw.map((group) => ({
    ...group,
    meta: vmExports.getHistoricalCategoryMeta(group.key),
  }));
  return {
    activeTableItems: vmExports.getActiveTableItems(payload),
    proxyTableItems: vmExports.getProxyTableItems(payload),
    nonActiveGroups: vmExports.getNonActiveGroups(payload),
...
function renderCanonicalPageModule(payload, metadata = {}) {
  const lines = [
    "/**",
    " * Auto-generated by fetch-contract-addresses.js",
    ` * Source: ${GOVERNOR_REPO} (${metadata.governorSha || "unknown"})`,
  ...
  for (const [key, value] of Object.entries(payload)) {
    lines.push(`export const ${key} = ${JSON.stringify(value, null, 2)};`);
    lines.push("");
  }
  lines.push(`export const canonicalContractsPageData = ${JSON.stringify(payload, null, 2)};`);
  lines.push("");
  lines.push("export default canonicalContractsPageData;");
  lines.push("");
  return lines.join("\n");
}
...
function buildDataArtifacts(data, blockchainPageData, governorSha, catalog) {
  ...
  const canonicalPagePayload = buildCanonicalPagePayload(payload);
  const canonicalJsonPayload = {
    _generated: {
      by: "fetch-contract-addresses.js",
      at: nowISO,
      source: `${GOVERNOR_REPO} (${governorSha})`,
  ...
  const canonicalContractsPageDataJsx = renderCanonicalPageModule(canonicalPagePayload, {
    generatedAt: nowISO,
    governorSha,
  });
  return {
    ...
    canonicalContractsPageData: canonicalPagePayload,
    files: {
      [OUTPUT_JSON_PATH]: {
        kind: "json",
        value: jsonPayload,
      ...
      [CANONICAL_PAGE_DATA_JSON_PATH]: {
        kind: "json",
        value: canonicalJsonPayload,
      },
      [CANONICAL_PAGE_DATA_JSX_PATH]: {
        kind: "jsx",
        value: canonicalContractsPageDataJsx,
      },
    },
  };
}
...
function writeDataFiles(data, blockchainPageData, governorSha, catalog, options = {}) {
  ...
  return {
    contractAddresses: artifacts.contractAddresses,
    blockchainContractsPageData: artifacts.blockchainContractsPageData,
    canonicalContractsPageData: artifacts.canonicalContractsPageData,
  };
}
