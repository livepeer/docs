 ...
 * @usage       node operations/scripts/dispatch/content/discoverability/dispatch-llms-files.js [--mode pr|scheduled|manual] [--dry-run|--write] [--verify]
...
function main() {
  ...
  if (args.help) { printPipelineHelp('dispatch-llms-files.js', 'llms-files'); process.exit(0); }
  const scope = scopeFlags(args);
  let code = 0;
  // Detect (drift check via --check flag for generator-style atomics)
