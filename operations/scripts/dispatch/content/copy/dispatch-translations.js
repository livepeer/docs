#!/usr/bin/env node
/**
 * @script      dispatch-translations
 * @type        dispatch
 * @concern     copy
 * @niche       translations
 * @purpose     Pipeline dispatcher for translations (full lifecycle: detect → repair → verify → escalate)
 * @description Pipeline for v2 translation freshness check and refresh.
 * @mode        dispatch
 * @pipeline    P3 (PR), P5/P6 (scheduled), manual
 * @scope       v2/es/, v2/fr/, v2/cn/
 * @usage       node operations/scripts/dispatch/content/copy/dispatch-translations.js [--mode pr|scheduled|manual] [--dry-run|--write] [--verify]
...
function main() {
  ...
  if (args.help) { printPipelineHelp('dispatch-translations.js', 'translations'); process.exit(0); }
