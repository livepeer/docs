...
const {
  parseArgs,
  runRepair
} = require('../../scripts/dispatch/governance/pipelines/governance-pipeline.js');
...
async function runTests() {
  ...
  cases.push(async () => {
    ...
    const targetPath = 'operations/scripts/example.js';
  ...
  cases.push(async () => {
    ...
    const preview = buildAuditReport({
      ...
      repair: {
        fixes: {
          total_fixes: 1,
          planned_files: ['operations/scripts/example.js']
        },
        projected_summary: buildSummary(),
        needs_human: [
          {
