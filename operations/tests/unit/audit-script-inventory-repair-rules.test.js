 ...
 * @scope             tests/unit, operations/scripts/validators/governance
 * @domain            docs
 * @needs             E-C1, R-R14
...
} = require('../../scripts/validators/governance/pr/audit-script-inventory.js');

let failures = [];

function makeScriptInfo(overrides = {}) {
  return {
    path: overrides.path || 'operations/scripts/example.js',
...
function main() {
  ...
  runCase('does not backfill judgement fields from classification JSON', () => {
    const scriptInfo = makeScriptInfo({
      path: 'operations/scripts/example.js'
    });
    ...
    assert.strictEqual(state.projected.usage, 'node operations/scripts/example.js [flags]');
  });
    ...
    const plan = buildRepairPlan(
      {
        classification_rows: [
          {
            path: 'operations/scripts/missing.js',
            script: 'missing'
          }
        ],
        scripts: []
