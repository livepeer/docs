...
|---|---|---|---|
| Health Audit Pre-PR | Ran all 6 health pipelines on docs-v2-dev-draft. Static all green: content-quality (0 critical), openapi (0), page-imports (855/855 valid), page-structure (analysed — real shipped findings tiny vs 2.9k mintignore'd-scratch noise). **page-links: fixed 295→40 broken internal refs** — 124 verified remaps (4 parallel resolution agents read source+target context) across 81 files/258 occurrences, 0 regressions. Residual 40 = 22 rss.xml feeds (flagged Changelog Pipeline) + 5 workload-fit (flagged Developers) + 6 DeepWiki artifacts + 1 frameworks card + 1 contracts verifier (flagged Contracts) + 5 non-issues. wcag + page-rendering blocked on local dev-server instability — verify in CI on the PR. Nothing committed. | Active — links fixed, render/wcag deferred to CI | 2026-05-29 |
