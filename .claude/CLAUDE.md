...
| Repo Features Documentation Audit | Two-track effort consolidated. **Track A** (`workspace/plan/active/REPO-FEATURES-DOCS-AUDIT/`): locked D-DG-01..D-DG-13 IA migration plan (features/ → reference/features/ etc.), source-of-truth matrix, feature verification with executable checks, framework/policy gap report, archive deletion queue, `check-docs-guide-reference-freshness.js` validator added. Validation blocked by 9 `lpd test --staged` errors: docs.json Resource HUB redirect/nav, root governance/allowlist (jsconfig.json), VSIX parity, repo governance sync, component usage-map. **Track B** (`workspace/thread-outputs/repo-docs-consolidation/`): 7 product-forward feature audits — AI (21 features), UI system (15), automations (16), data integrations (11), adaptive architecture (23), contributor tools (14), RFP/internal (17 pages). Master synthesis at `00-SYNTHESIS.md` covers 5 cross-cutting patterns (claims-drift, governance-spec-drift, three-surface duplication, persistent incomplete features, self-violation) + 80+ community-help opportunities with file paths + 5-phase RFP refresh plan. Live counts (per Track A): 55 workflows, 264 scripts, 35 SKILL.md, 132 components in registry, 541 v2 routes in docs.json. Verified: `.env` in workspace/plan/active/SOLUTIONS-SOCIAL-DATA/ exists locally with real keys but is gitignored AND never committed (clear). | Active — Session 1 done, Session 2 awaiting approval | 2026-05-18 |
