...
| **Report path** | `tasks/reports/27-pre-commit-full-browser-verification-report.md` (create on completion) |
...
- Update `contribute/CONTRIBUTING/GIT-HOOKS.md` to describe the new behaviour and that the check must not be skipped.

## Deliverables

- Updated pre-commit hook (and test runner if needed) so browser verification runs and is not skippable for pre-commit.
- Documentation in `contribute/CONTRIBUTING/GIT-HOOKS.md` (what runs, smoke set if used, and that the check must not be skipped).
- Report at `tasks/reports/27-pre-commit-full-browser-verification-report.md` with implementation details, trade-offs, and follow-ups.
