...
<Info>
  Livepeer Studio has its own full product documentation. This page covers Studio as a gateway provider in the context of the Livepeer network. For complete product guides, visit the [Livepeer Studio product section](/v2/platforms/products/livepeer-studio/overview/overview) or [livepeer.studio](https://livepeer.studio).
</Info>
...
- **AI inference** — Access the Livepeer AI network via Studio's `/api/beta/generate/` endpoints (text-to-image, image-to-image, image-to-video, and more)
...
Broadcast via RTMP to `rtmp://rtmp.livepeer.com/live` using the stream key. Play back via HLS at `https://livepeercdn.studio/hls/{playbackId}/index.m3u8` or with the Livepeer React Player.
...
const livepeer = new Livepeer({
  httpBearer: process.env.GATEWAY_API_KEY,
});
...
<Note>
  For AI inference via Livepeer Studio, use `livepeer.studio/api/beta/generate/` as your base URL with your Studio API key as the bearer token.
</Note>
...
<CardGroup cols={2}>
  <Card title="Studio Product Docs" href="/v2/platforms/products/livepeer-studio/overview/overview" icon="video-arrow-up-right" arrow>
  ...
  <Card title="AI Quickstart" href="/v2/developers/building-on-livepeer/quick-starts/livepeer-ai" icon="robot" arrow>
