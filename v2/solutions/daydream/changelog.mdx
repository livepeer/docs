...
<Update label="Scope: v0.2.2" tags={["Features", "Fixes", "Scope"]} rss={{ title: "Daydream Scope v0.2.2", description: "This release introduces Multi Source / Multi Sink graphs, an Audio In node, and a number of bug fixes across graph mo..." }} description={<Subtitle variant="changelog">April 2026</Subtitle>}>
  ...
  * **Multi Source / Multi Sink Graphs** – Workflows can now contain multiple source and sink nodes in a single graph. Each source routes to its own input track, each sink shows its own per-sink stats (FPS / bitrate), and per-node recording lets you capture each sink independently. Works in local, cloud, and headless modes
  * **Audio In Node** – A new graph node for audio inputs. Upload WAV, MP3, FLAC, or OGG files to the asset library, pick them from a new audio media picker, and forward audio into your pipelines

  #### Updates

  * **Keyboard Shortcuts & Fullscreen** – New keyboard shortcuts and a fullscreen button on the Sink node improve navigation in the Workflow Builder
  * **External Parameter Sync** – OSC, REST, and MCP parameter updates now sync correctly in graph mode
  * **LoRA Merge Strategy View** – You can now view and manage LoRA merge strategies directly in the Workflow Builder
  * **Headless MPEG-TS Streaming** – A new headless streaming endpoint for MPEG-TS output

  #### Bug fixes

  * Fixed cloud relay routing and connection error handling
  * Fixed NDI / Syphon source issues in multi-sink graphs
  * Fixed recording timestamps and per-sink recording
  * Fixed integer handling and prompt management in graph mode
  * Improved plugin listing performance after cloud connect
