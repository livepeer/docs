...
The Livepeer Network is a posted-price market for GPU compute.
- Operators on the supply side publish their capabilities and prices.
- Gateways on the demand side discover, score, and route work to them.

Payment runs continuously through probabilistic micropayments, with on-chain settlement only when a ticket wins.

The marketplace runs continuously off-chain so per-pixel video and per-token AI remain economical at production volume.
...
<BorderedBox variant="accent">
  <Subtitle fontSize="1rem" marginBottom="0.5rem">**Visible on-chain**</Subtitle>
  ...
  <Subtitle fontSize="1rem" marginTop="1rem" marginBottom="0.5rem">**Off-chain only**</Subtitle>
...
The Network produces verifiable work without a central operator via stake exposure, fast verification, and the network discovery surface.
