...
For transcoding, `-pricePerUnit` is set in wei per pixel at node startup rather than in `aiModels.json`.
...
Pricing is orchestrator-set and discoverable at session time. The [orchestrator selection](/v2/developers/guides/payments/orchestrator-selection-and-pricing) page covers how gateways filter by price.
