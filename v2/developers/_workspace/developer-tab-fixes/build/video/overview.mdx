...
See <LinkArrow href="/v2/developers/build/video/ingest" label="Ingest" newline={false} /> for ingest configuration, webhook setup, and stream key management.
...
See <LinkArrow href="/v2/developers/build/video/vod" label="VOD" newline={false} /> for upload, status polling, and playback integration.
...
See <LinkArrow href="/v2/developers/build/video/transcoding" label="Transcoding" newline={false} /> for the segment submission API and profile configuration.
...
Start with [ingest](/v2/developers/build/video/ingest) for live streams or [VOD](/v2/developers/build/video/vod-and-recording) for asset upload. Both produce a `playbackId` usable with the `@livepeer/react` [Player](/v2/developers/build/video/player).
...
<CardGroup cols={2}>
  ...
  <Card title="LPMS" icon="server" href="/v2/developers/build/video/lpms" arrow horizontal>
