...
<Note>
  LPMS is the media layer embedded in go-livepeer. For the full Livepeer protocol with orchestrator discovery, probabilistic micropayments, and network routing, run go-livepeer in broadcaster mode rather than using LPMS directly.
</Note>

<CustomDivider />

LPMS is the media layer embedded in go-livepeer. For the full protocol with orchestrator discovery and payments, run go-livepeer in broadcaster mode rather than using LPMS directly.
