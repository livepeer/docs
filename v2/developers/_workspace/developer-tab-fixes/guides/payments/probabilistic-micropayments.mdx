...
The PM mechanism handles settlement automatically inside go-livepeer. Developers using the REST API or SDKs never interact with tickets directly. PM matters when operating a [self-hosted gateway](/v2/developers/guides/local-development/local-gateway) or building a [clearinghouse](/v2/developers/guides/payments/clearinghouse-pattern).
