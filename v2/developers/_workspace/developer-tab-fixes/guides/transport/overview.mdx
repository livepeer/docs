...
Most developers need only RTMP (for ingest from encoders) and HLS (for playback). WebRTC matters when sub-second latency is required. Trickle is internal to real-time AI and handled by ComfyStream and PyTrickle automatically.
