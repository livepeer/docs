...
description: >-
  Set up the Livepeer API webhooks to receive stream and asset events. Covers endpoint setup, signature verification, event types, and troubleshooting.
keywords:
  - livepeer webhooks
  - stream events
  - asset events
...
Studio's webhook dashboard at https://the Livepeer API/dashboard/developers/webhooks shows delivery history and lets you resend failed events.
...
The `payload` object shape varies by event type. Refer to the [Studio API Reference](https://the Livepeer API/docs) for per-event schemas.
