...
Every request to the Livepeer AI Gateway API requires a Bearer token. Obtain one from [the Livepeer API](https://livepeer.com) under **Settings > API Keys**. Two key types exist -- choose based on where your code runs.
...
```bash
curl -X POST https://livepeer.com/api/beta/generate/text-to-image \
...
When your frontend makes direct API calls, create a CORS-enabled key in Studio. The developer dashboard scopes the key to specific asset or stream IDs -- a leaked CORS key cannot access other resources in your account.
