...
<CenteredContainer style={{ width: '90%' }}>
  <Tip>The Video SDK covers all Livepeer resources (streams, assets, and AI inference) in one package. The AI SDK covers inference only. For most applications, use the Video SDK.</Tip>
</CenteredContainer>

<CustomDivider />

Livepeer provides two SDK families. The **Video SDK** (`livepeer`) wraps the full Livepeer REST API: livestreams, video assets, webhooks, access control, and AI inference. The **AI SDK** (`@livepeer/ai`) wraps AI inference endpoints only. Both authenticate with a Bearer token from the developer dashboard.
...
<Tabs>
  <Tab title="TypeScript">
    ...
    const client = new Livepeer({
      apiKey: process.env.LIVEPEER_API_KEY, // Bearer token
    });
    ```
  </Tab>
  <Tab title="Python">
  ...
  <Tab title="AI SDK (TypeScript)">
    ...
    // For production: configure your gateway URL
    ```
  </Tab>
</Tabs>
...
Two API key types exist in the dashboard. Use the correct type for your deployment context.
...
<StyledTable variant="bordered">
  ...
  <tbody>
    ...
    <TableRow>
      ...
      <TableCell>Scoped to asset and stream IDs. Safe to embed in frontend. Create in developer dashboard under API Keys.</TableCell>
    </TableRow>
  </tbody>
</StyledTable>
...
Use the Video SDK for applications that need streams, assets, and inference. Use the AI SDK for inference-only applications. Both handle auth, retries, and typed responses automatically.

## Related pages

<CardGroup cols={2}>
  <Card title="AI Quickstart" icon="bolt" href="/v2/developers/build/ai-and-agents/ai-jobs-direct-quickstart">
    First AI API call in 10 minutes.
  </Card>
  <Card title="SDKs Reference" icon="book" href="/v2/developers/resources/reference/sdks">
    All SDK versions, packages, and links to full API reference.
  </Card>
  <Card title="AI Authentication" icon="lock" href="/v2/developers/guides/auth-and-security/ai-authentication">
    API key types, rotation strategy, and CORS configuration.
  </Card>
  <Card title="AI Troubleshooting" icon="triangle-exclamation" href="/v2/developers/guides/observability-and-debugging/job-debugging">
