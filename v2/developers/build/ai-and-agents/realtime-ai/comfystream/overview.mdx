...
<CustomDivider middleText="Pipeline Modes" />
...
<CustomDivider middleText="Node Ecosystem" />
...
<CustomDivider middleText="Workflow Format" />
...
<CustomDivider middleText="Data Channel" />
...
To receive data-channel output from a browser client, use <LinkArrow href="" label="" newline={false} />, which handles WebRTC video streaming and the data channel from the same connection.
...
<CustomDivider middleText="Hardware" />
...
VRAM headroom matters for stability. A workflow that runs at 12 GB may stutter under load that 16 GB absorbs cleanly. Source: the <LinkArrow href="" label="" newline={false} />.

CUDA 12.0+ is required. Current ComfyStream releases target CUDA 12.8 with NVIDIA driver 570.124.06 or later.

<CustomDivider middleText="Relationship to BYOC" />
...
| Run a custom Python model that isn't a ComfyUI workflow | <LinkArrow href="" label="" newline={false} /> |
