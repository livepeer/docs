...
<CustomDivider middleText="Node Categories" />
...
<CustomDivider middleText="Control Nodes" />
...
Both nodes replace the static `Load Image` pattern used in batch ComfyUI workflows. In a streaming context, they receive frames from the ComfyStream server on each forward pass rather than reading from disk.
...
<CustomDivider middleText="Usage Pattern" />
...
ComfyStream's server intercepts the `LoadTensor` node at runtime and injects decoded video frames. The downstream nodes process each frame as they would a static image input, but the forward pass is called once per incoming frame rather than once per workflow execution.

See <LinkArrow href="/v2/developers/build/ai-and-agents/comfystream/workflow-authoring" label="Workflow Authoring" newline={false} /> for the full workflow setup and streaming server configuration.
