...
- your model is an LLM you want to run via Ollama rather than the standard `livepeer/ai-runner` image. The
  same overall flow applies but the runner image and `aiModels.json` entry differ. See the LLM variant note
  at the end.

<CustomDivider middleText="Pipelines" />
...
<CustomDivider middleText="Step 1: Model directory" />
...
<CustomDivider middleText="Step 2: Declare the model" />
...
<CustomDivider middleText="Step 3: Download weights" />
...
<CustomDivider middleText="Step 4: Start orchestrator" />
...
<CustomDivider middleText="Step 5: Verify capability" />
...
<CustomDivider middleText="Step 6: Inference test" />
...
<CustomDivider middleText="Step 7: Confirm the loop" />
...
<CustomDivider middleText="LLM variant" />
...
<AccordionGroup>
  ...
  <Accordion title="Localhost works but gateway fails" icon="globe">
    ...
    confirm DNS, confirm the orchestrator is binding to a public interface and not just `localhost`.
  </Accordion>
...
<CardGroup cols={2}>
  <Card title="Advanced HuggingFace paths" icon="code-branch" href="/v2/developers2/build/tutorials/huggingface-to-livepeer-advanced" arrow horizontal>
  ...
  <Card title="ComfyStream Quickstart" icon="bolt" href="/v2/developers2/build/tutorials/comfystream-quickstart" arrow horizontal>
