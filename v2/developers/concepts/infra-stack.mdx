---
title: Persona infrastructure maps
sidebarTitle: Infra Stack
description: "Maps the infrastructure each of the five developer personas touches and the guides each persona needs."
pageType: concept
audience: developer
purpose: explain
...
{/* MIGRATION NOTE (moved from developers1/concepts/diagrams.mdx on 2026-05-12).
    Violations to resolve on next pass:
    - Two duplicate H1 lines stripped (`# Personas` + `# Persona infrastructure maps...`).
      Frontmatter `title:` now renders as the H1 per Mintlify convention.
    - Source contained 12 em-dashes – most in Mermaid label text and section headings.
      Replace narrative ones with colons/hyphens. Mermaid label em-dashes are accepted.
    - Frontmatter was 1/8 complete in source; now populated. Verify keywords match search intent.
    - Persona-model lock (Task 1 §F.1 Option A confirmed): close at 5 personas. Add subsumption notes –
      Persona E (SDK/Alt-Gateway Builder) subsumed under P3 Compute Primitives;
      Persona F (Agent-Runtime Developer) subsumed under P1 AI.
*/}
