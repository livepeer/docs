...
{/* MIGRATION NOTE (moved from developers1/learn/applications-on-livepeer.mdx on 2026-05-12).
    Violations to resolve on next pass:
    - Page TITLE is "When to Run Your Own Gateway" but IA target `learn/applications.mdx` should
      cover how network-direct surfaces (AI Jobs API + self-hosted gateway + BYOC + ComfyStream + ui-kit)
      compose into an application. Scope mismatch – content needs full rewrite or scope-split.
      Existing gateway-decision content moves to `guides/gateways-as-developer/self-hosted-decision.mdx`.
    - Reference to "Livepeer Studio" in the opening paragraph: replace with "Cloud SPE community gateway"
      per no-Studio-content rule. Studio is acknowledged via `learn/where-to-find/studio-paths.mdx` only.
*/}
