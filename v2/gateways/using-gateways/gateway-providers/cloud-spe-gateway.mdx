...
- **Prereqs:** A backend runtime for API calls and a Cloud SPE-compatible API key if required by the current provider policy
- **Time:** 5 minutes
- **Outcome:** AI gateway endpoint discovered and first authenticated request scaffolded
- **First action:** Open [tools.livepeer.cloud](https://tools.livepeer.cloud), copy the current AI base URL/auth requirements, then run a test request
...
```bash
CLOUD_SPE_AI_BASE="<copy from tools.livepeer.cloud>"

curl -X POST "$CLOUD_SPE_AI_BASE/text-to-image" \
  -H "Authorization: Bearer <LIVEPEER_GATEWAY_API_KEY>" \
