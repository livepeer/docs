...
description: >-
  Choose the right gateway deployment path for your use case. Covers deployment modes,
  node types, five setup paths, and community tooling.
pageType: guide
audience: gateway
purpose: orient
status: deprecated
...
<AccordionGroup>
  ...
  <Accordion title="Path 3 - Dual Gateway (AI + Video): Both workloads on one node" icon="circle-half-stroke">
    **This is the right path if:** You want to serve both AI inference requests and video transcoding from a single gateway instance, or you want to add AI pipeline capabilities alongside an existing transcoding setup.
    ...
    <CardGroup cols={2}>
      <Card title="Configure dual gateway" icon="gear" href="/v2/gateways/setup/configure" arrow>
