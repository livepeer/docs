...
```mermaid
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#EAF7EB', 'primaryTextColor': '#111827', 'primaryBorderColor': '#3CB540', 'lineColor': '#2B9A66', 'secondaryColor': '#F4FBF5', 'tertiaryColor': '#FFFFFF', 'clusterBkg': '#F4FBF5', 'clusterBorder': '#3CB540', 'edgeLabelBackground': '#FFFFFF'}}}%%
graph LR
    A[Discovery] --> B[Truth]
    B --> C[Provenance]
    C --> D[Presentation]
...
|---|---|---|---|
| Tier 1 | Controller `SetContractInfo` events on Arbitrum and Ethereum | New or changed controller-managed entries | Fast signal that current protocol state changed |
| Tier 1 | Full repo watch on `livepeer/protocol`, `livepeer/arbitrum-lpt-bridge`, `livepeer/go-livepeer`, `livepeer/governor-scripts` | New branches, default-branch changes, contract-related commits | Finds new contract families and provenance changes |
| Tier 1 | Commit diff scan under `contracts/`, `deploy/`, `deployments/`, `tasks/`, `cmd/livepeer/`, `eth/` | Contract-introducing changes | Narrows the scope of review |
| Tier 2 | `governor-scripts/updates/addresses.js` changes | Drift or operational changes in the human-maintained manifest | Useful trigger and comparison signal, not publish truth |
...
| Tier 1 | Bridge getter graph plus official bridge deployment artefacts | Bridge cluster families | Linked current bridge addresses |
| Tier 1 | Official deployment artefacts plus official runtime consumers such as `go-livepeer` | Detached non-controller families | Current address and real runtime usage |
...
The trigger model combines scheduled verification, manual inspection, and event-driven upstream changes.

| Trigger | Scope | Purpose |
|---|---|---|
| `schedule` | Daily or every 6 hours | Catch missed changes and refresh verification |
| `workflow_dispatch` | Manual | Force a dry run, inspection run, or targeted verification |
| `repository_dispatch` | Upstream repo events | Re-run when official sources change |
| Branch inventory refresh | Inside scheduled and event-driven runs | Detect new branches or default-branch changes without guessing branch truth |
...
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#EAF7EB', 'primaryTextColor': '#111827', 'primaryBorderColor': '#3CB540', 'lineColor': '#2B9A66', 'secondaryColor': '#F4FBF5', 'tertiaryColor': '#FFFFFF', 'clusterBkg': '#F4FBF5', 'clusterBorder': '#3CB540', 'edgeLabelBackground': '#FFFFFF'}}}%%
graph TD
    A[Known controller slot name] --> B[getContractInfo bytes32]
    A --> C[getContract bytes32]
    B --> D[Current address and gitCommitHash]
    C --> E[Current address cross-check]
    D --> F{Proxy family}
    E --> F
    F -->|Yes| G[proxy controller]
    G --> H[targetContractId]
    H --> I[controller getContract targetId]
    I --> J[Current implementation]
    F -->|No| K[Direct current address]
    B --> L[Arbitrum SetContractInfo history]
    L --> M[Historical series where authoritative]
    D --> N[Current truth decision]
    J --> N
    K --> N
```
...
Bridge cluster contracts are not registered in the Controller. Their current addresses are established from official upstream deployment artefacts and cross-verified through the bridge getter graph. The Ethereum `Minter` slot, from Group 1, seeds the cluster entry point.
...
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#EAF7EB', 'primaryTextColor': '#111827', 'primaryBorderColor': '#3CB540', 'lineColor': '#2B9A66', 'secondaryColor': '#F4FBF5', 'tertiaryColor': '#FFFFFF', 'clusterBkg': '#F4FBF5', 'clusterBorder': '#3CB540', 'edgeLabelBackground': '#FFFFFF'}}}%%
graph TD
    A[Ethereum Minter slot] --> B[BridgeMinter seed address]
    C[Bridge deployment artefacts] --> D[L1LPTGateway artefact]
    C --> E[L1Migrator artefact]
    C --> F[L1Escrow artefact]
    C --> G[L2LPTGateway artefact]
    B --> H[BridgeMinter l1LPTGatewayAddr]
    B --> I[BridgeMinter l1MigratorAddr]
    D --> J[L1LPTGateway l1LPTEscrow]
    D --> K[L1LPTGateway l2Counterpart]
    G --> L[L2LPTGateway l1Counterpart]
    H --> M[Compare to L1LPTGateway artefact]
    I --> N[Compare to L1Migrator artefact]
    J --> O[Compare to L1Escrow artefact]
    K --> P[Compare to L2LPTGateway artefact]
    L --> Q[Compare back to L1LPTGateway]
    M --> R[Bridge truth decision]
    N --> R
    O --> R
    P --> R
    Q --> R
...
2. Confirm current usage from an official runtime consumer where available.
...
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#EAF7EB', 'primaryTextColor': '#111827', 'primaryBorderColor': '#3CB540', 'lineColor': '#2B9A66', 'secondaryColor': '#F4FBF5', 'tertiaryColor': '#FFFFFF', 'clusterBkg': '#F4FBF5', 'clusterBorder': '#3CB540', 'edgeLabelBackground': '#FFFFFF'}}}%%
graph TD
    A[Official deployment artefact] --> B[Detached contract address]
    B --> C[Official runtime consumer]
    C --> D{Runtime confirms same address}
    B --> E[Explorer source and deployer metadata]
    D -->|Yes| F[Detached verification record]
    D -->|No| G[Contradiction]
    E --> F
    F --> H[Human review gate]
    H --> I[Publish if approved]
    G --> J[Fail and open issue]
```
...
|---|---|---|---|
| Arbitrum RPC primary and fallbacks | `https://arb1.arbitrum.io/rpc`  `https://arbitrum-one-rpc.publicnode.com`  `https://arbitrum.drpc.org` | `eth_call` results for `getContract(bytes32)`, proxy `controller()`, proxy `targetContractId()`, controller `getContract(targetId)`; target design also uses `eth_getLogs` for Arbitrum event history | provider outage, rate limiting, inconsistent archive/log availability, provider disagreement on current truth |
| Ethereum RPC primary and fallbacks | `https://eth.llamarpc.com`  `https://ethereum-rpc.publicnode.com`  `https://eth.drpc.org` | `eth_call` results for current Ethereum controller state and bridge-seed lookups; target design may also probe `eth_getLogs` for historical availability checks | provider outage, incomplete event history, rate limiting, provider disagreement on current truth |
...
| Docs-local truth influencing a current address | Treat as design violation, not transient error | Halt immediately | Write anomaly report and open GitHub issue |
| Non-active row leaking into the active surface | Treat as classification failure | Halt immediately | Write anomaly report and open GitHub issue |
...
- failure class
- affected contract family and chain where applicable
...
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#EAF7EB', 'primaryTextColor': '#111827', 'primaryBorderColor': '#3CB540', 'lineColor': '#2B9A66', 'secondaryColor': '#F4FBF5', 'tertiaryColor': '#FFFFFF', 'clusterBkg': '#F4FBF5', 'clusterBorder': '#3CB540', 'edgeLabelBackground': '#FFFFFF'}}}%%
graph TD
    A[Failure detected] --> B{Infrastructure or truth failure}
    B -->|Transient infrastructure| C[Retry with bounded backoff]
    C --> D{Recovered cleanly}
    D -->|Yes| E[Continue run]
    D -->|No| F[Use fallback provider or secondary service]
    F --> G{Resolved without contradiction}
    G -->|Yes| E
    G -->|No| H[Block publication]
    B -->|Truth contradiction or design violation| H
    H --> I[Write anomaly report]
    I --> J[Create or update GitHub issue]
    J --> K[End run non-zero]
```
...
| Duplicate incident on repeated runs | Update existing issue using the incident fingerprint instead of opening a new one | New contradictory evidence changes severity or scope |

Self-remediation outputs:

- update step summary with exact recovery path attempted
- attach anomaly JSON or Markdown artifact to the run
- create or update a GitHub issue with labels such as `contracts`, `pipeline`, `verification`, and the failure class
...
- accept `governor-scripts` as sole truth because the chain lookup failed
- replace unresolved provenance with a guessed branch link
- publish stale previous output as if it were freshly verified
- silently suppress a contract from the active set because verification became inconvenient

```mermaid
%%{init: {'theme': 'base', 'themeVariables': {'primaryColor': '#EAF7EB', 'primaryTextColor': '#111827', 'primaryBorderColor': '#3CB540', 'lineColor': '#2B9A66', 'secondaryColor': '#F4FBF5', 'tertiaryColor': '#FFFFFF', 'clusterBkg': '#F4FBF5', 'clusterBorder': '#3CB540', 'edgeLabelBackground': '#FFFFFF'}}}%%
graph TD
    A[Pipeline problem] --> B{Safe to retry without weakening truth}
    B -->|Yes| C[Retry primary service]
    C --> D{Still failing}
    D -->|No| E[Resume run]
    D -->|Yes| F[Try fallback provider or cached inventory path]
    F --> G{Fallback agrees and resolves}
    G -->|Yes| E
    G -->|No| H[Stop publication]
    B -->|No| H
    H --> I[Attach anomaly artifact]
    I --> J[Create or update GitHub issue]
    J --> K[Keep outputs blocked]
```
...
| Need | How it will be handled | Exact implementation names and flow |
|---|---|---|
| Dedicated RPC strategy for CI | Production CI uses dedicated Arbitrum and Ethereum RPC endpoints from secrets, with a fallback provider for retry and comparison. Public RPC remains acceptable for local development only. If primary and fallback providers disagree on current address truth, the run fails and emits an RPC anomaly report. | Workflow: `.github/workflows/update-contract-addresses.yml` replacement. Script: `.github/scripts/fetch-contract-addresses.js` replacement. Env vars: dedicated `ARBITRUM_RPC_URL`, `ARBITRUM_RPC_FALLBACK_URL`, `ETHEREUM_RPC_URL`, `ETHEREUM_RPC_FALLBACK_URL`. Flow: workflow injects secrets -> script uses primary RPC for `eth_call` and `eth_getLogs` -> on failure or contradiction script queries fallback RPC -> script writes `_health-checks.json` and anomaly artifact -> workflow opens or updates a GitHub issue if the run still cannot establish current truth. |
| Mechanical validation of all published explorer links | The pipeline generates explorer URLs from a fixed chain-to-host mapping, then validates every published link for correct host, correct `/address/` path, and exact published address value. The generated health checks record those validations. A bad explorer host, malformed path, or wrong address link is a blocking failure. | Generator output fields: `blockchainHref`, `verification.explorer.addressUrl`, and health checks in `snippets/data/contract-addresses/_health-checks.json`. Flow: `fetch-contract-addresses.js` builds links from canonical chain mapping -> validator checks host is `arbiscan.io` or `etherscan.io` as appropriate -> validator checks address path exactly matches the published address -> failing rows are added to anomaly output and block publication. |
| Strong page and widget copy that explain the real proof chain | The page and widget read from the same verification model used by the generated data. Controller-registered, proxy, bridge, and detached proof paths are described from the actual evidence fields. Copy that simplifies the source model beyond the real proof chain is a content defect. | Data source: `snippets/data/contract-addresses/contractAddressesData.json` and `snippets/composables/pages/canonical/livepeer-contract-addresses-data.json`. Consumers: `snippets/composables/pages/canonical/livepeer-contract-addresses.mdx` and `snippets/components/integrators/feeds/ContractVerifier.jsx`. Flow: generator writes structured verification fields such as `verification.controller`, `verification.proxy`, `addressSource`, and `codeSource` -> page accordion and widget copy render from those fields -> review pass checks wording against the generated model before publication. |
| Presentation that supports SEO, AEO, and human self-verification | The public surface keeps one canonical route, one machine-readable data output, current explorer links, commit-specific source-code links, and verification signals that a human can follow independently. | Canonical surface: `/v2/about/resources/livepeer-contract-addresses`. Machine-readable outputs: `contractAddressesData.json` and `livepeer-contract-addresses-data.json`. Supporting consumers: `livepeer-contract-addresses.mdx`, `ContractVerifier.jsx`, and `blockchain-contracts.mdx`. Flow: generator writes active-only public data + separate implementation and secondary inventory data -> canonical page exposes current address, source-code link, and explorer link -> widget provides lookup and address verification using the same generated fields -> docs copy stays aligned with the real verification chain. |
