...
import { CustomCallout, BlinkingIcon, BlinkingTerminal, DoubleIconLink, GotoLink, GotoCard, TipWithArrow, LinkArrow } from '/snippets/components/primitives/links.jsx'
...
| `color` | `string` | theme accent | No | Color of the icon (defaults to theme accent) |
...
<Tabs>
  <Tab title="Live Indicator">
    <div style={{ display: "flex", gap: "1rem", alignItems: "center" }}>
      ...
      <BlinkingIcon icon="signal" color="#3b82f6" size={16} /> Active
      <BlinkingIcon icon="terminal" size={20} /> Processing
    </div>
  </Tab>
  <Tab title="Code">
```jsx
...
**Note:** Deprecated - use `BlinkingIcon` instead.
