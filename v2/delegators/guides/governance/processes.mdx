...
<Note>
  **Orchestrators: vote directly from the CLI.** Orchestrators running `go-livepeer` can vote on both governance polls and treasury proposals using `livepeer_cli`, without importing their wallet into a browser. This is the recommended, more secure workflow — your private keys stay on the node. See [Network Participation](/v2/orchestrators/guides/staking-and-rewards/network-participation#how-to-vote) for step-by-step instructions.
</Note>
