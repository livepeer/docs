...
Historically Livepeer distributed inflationary rewards to orchestrators and their delegators, while ETH fee sharing was governed by each orchestrator's configured fee split. Proposals to introduce a broader protocol-level fee switch would require a LIP and fresh economic analysis. The discussion remains part of the protocol's design space, but delegator docs should treat any such change as governance-dependent rather than assumed current behaviour.
