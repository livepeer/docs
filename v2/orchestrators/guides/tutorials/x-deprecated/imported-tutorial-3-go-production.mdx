...
*/}
<Warning>
This placeholder page previously imported a routed Gateway tutorial directly, which breaks the repository's Mint composition contract.

Use the canonical Gateway tutorial instead:
- [Go production](/v2/gateways/guides/tutorials/tutorial-3-go-production)

If this orchestrator tutorial needs to return as a first-class page, extract any shared walkthrough sections into `snippets/composables/**` and compose both routed pages from that shared source.
</Warning>

{/*
  PURPOSE:
  Go-production tutorial. Currently a minimal import.

  PLAN TARGET: Replace with ai-earning-quickstart or add-ai-to-video-node tutorial
  STATUS: MINIMAL IMPORT - to be replaced with orchestrator-specific content

  SECTION: Tutorials → "Show me"
  JOB STORIES: J6 (AI quickstart) or J2 (add AI to video)
*/}
