...
**For supported models and pipeline compatibility:** The [AI Model Support](/v2/developers/build/ai-and-agents/model-support) page in the Developers section lists every pipeline type, supported model architectures, minimum VRAM, and current network status. This is the single source of truth for "will this model work on the network?" Use it before experimenting with untested model IDs.
...
**For custom models outside the standard pipeline list:** [Bring Your Own Container (BYOC)](/v2/developers/build/compute/byoc/overview) covers building a custom Docker container with PyTrickle integration to run any model on the network. BYOC is the path for proprietary models, fine-tuned checkpoints, or models with non-standard inference architectures.
...
<CardGroup cols={2}>
  <Card title="AI Model Support" icon="list-check" href="/v2/developers/build/ai-and-agents/model-support">
  ...
  <Card title="Bring Your Own Container (BYOC)" icon="box-open" href="/v2/developers/build/compute/byoc/overview">
