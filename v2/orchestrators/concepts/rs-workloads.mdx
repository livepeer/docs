...
<Note>
  ...
  Pipelines](https://docs.livepeer.org/ai/pipelines/overview) for current per-pipeline specs.
</Note>
...
Configured via the `aiModels.json` file. Each entry specifies the pipeline name, model ID, pricing, whether the model is kept warm, and the URL of the runner container. See [Configure AI Pipelines](/v2/orchestrators/operations/rc-ai-pipelines) {/* REVIEW: confirm correct target path */} for the configuration reference.
...
Configured via `aiModels.json` and requires a running ComfyStream instance. See [Set Up Real-time AI Pipelines](/v2/developers/get-started/comfystream-quickstart) {/* REVIEW: confirm target path */} for the setup guide.
...
<CardGroup>
  <Card
    ...
    icon="map"
    href="/v2/orchestrators/quickstart/x-setup-paths"
  >
  ...
  <Card
    ...
    icon="cpu"
    href="/v2/orchestrators/quickstart/AI-prompt-start"
  >
...
5. **`/v2/orchestrators/operations/rc-ai-pipelines` and `/v2/orchestrators/setup/llm-pipeline`** — these are target links from this page; confirm they are being created as part of Phase 1 content work
