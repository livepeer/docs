# Review: AI Earning Quickstart
**File:** v2/orchestrators/guides/tutorials/ai-earning-quickstart.mdx
**Reviewed:** 2026-04-08

## Summary
| Category | Check # | Result | Evidence |
|---|---|---|---|
| Frontmatter | 1.4 | PASS | pageType: tutorial |
| Frontmatter | 1.6 | FAIL | Missing purpose |
| Frontmatter | 1.7 | FAIL | Missing complexity |
| Frontmatter | 1.8 | FAIL | Missing lifecycleStage |
| Frontmatter | 1.13 | WARN | status: "current" |
| Voice | 2.1 | PASS | UK English |
| Structure | 4.1 | PASS | Single purpose: AI quickstart |
| Layout | 5.1 | PASS | Tutorial template, StyledSteps, tables |
| Nav | 7.1 | PASS | In docs.json |

## Frontmatter Table
| Field | Present? | Value | Pass/Fail |
|---|---|---|---|
| title | Yes | AI Earning Quickstart | PASS |
| sidebarTitle | Yes | AI Quickstart | PASS |
| description | Yes | 93 chars | PASS |
| pageType | Yes | tutorial | PASS |
| audience | Yes | orchestrator | PASS |
| purpose | No | — | FAIL |
| complexity | No | — | FAIL |
| lifecycleStage | No | — | FAIL |
| keywords | Yes | 8 items | PASS |
| og:image | Yes | orchestrators.png | PASS |
| status | Yes | current | WARN |

## Verdict: NEEDS WORK
Good quickstart tutorial. Fix: 3 missing frontmatter fields.
