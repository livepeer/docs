...
where orchestrators sit in the Livepeer stack, how they interact with other network participants, how work flows through the system, and the key components involved.
...
- **Registration** — `livepeer_cli` submits a bond transaction to register - **Reward calls** — once per round (~24 hours), the node calls `reward()` to mint new LPT
...
- Activation (registering staking LPT)
