...
- `v2/orchestrators/guides/advanced-operations/gateway-orchestrator-interface.mdx`
  - claim families: none
  - extracted: Running both roles on one machine is supported but requires deliberate port separation.
  - extracted: Running both a gateway and an orchestrator under your control gives full visibility into both sides of the job pipeline, allows you to test your own inference, and enables serving your own products without depending on third party orchestrators.
  - extracted: The orchestrator runs with full on chain registration and stake.
  - extracted: This pattern suits operators with a single powerful machine who want to eliminate network latency between gateway and orchestrator.
- `v2/orchestrators/guides/advanced-operations/gateway-relationships.mdx`
  - claim families: none
  - extracted: Understanding this pipeline is what separates operators who consistently receive work from those who are technically running but idle.
  - extracted: Your on chain registration — service URI, stake, cuts — makes you visible here automatically once you are in the active set.
  - extracted: What this means for you: For standard video transcoding, being registered on chain and in the top 100 active orchestrators is sufficient for discovery.
  - extracted: For AI workloads, capability advertisement through your configuration is what makes you discoverable for specific pipelines.
- `v2/orchestrators/guides/advanced-operations/pool-operators.mdx`
  - claim families: none
  - extracted: You hold the on chain identity and LPT stake; workers contribute GPU compute and earn payouts managed off chain by you, the operator.
  - extracted: It has a single on chain address, a single stake, and a single service URI.
  - extracted: All stake, all protocol reputation, and all on chain fees flow to and through your orchestrator address.
  - extracted: Worker provides: A machine with an NVIDIA GPU, NVIDIA drivers, and network connectivity to your orchestrator on port 8935.
- `v2/orchestrators/guides/advanced-operations/scale-operations.mdx`
  - claim families: none
  - extracted: You are likely in fleet territory if: Your workload requires more GPU capacity than fits in one machine You need geographic distribution for latency sensitive AI workloads You want to separate concerns — one node for reward calling, one for ticket redemption, separate GPU workers You are operating at data centre scale with SLA commitments to gateway operators If you are not clearly in one of these situations, start with a single node.
  - extracted: Node separation allows you to assign specific functions to specific nodes: Separation patterns: Node role What it does Why separate it Workload nodes Process transcoding and AI inference jobs Scale horizontally; restart without affecting rewards Reward node Calls each round Reward safety — dedicated stable machine, not affected by workload disruption Redeemer node Redeems winning payment tickets High volume ticket processing on dedicated hardware This pattern is architecturally similar to the — the principle of separating reward calling from workload processing is the same, but implemented entirely within go livepeer rather than using OrchestratorSiphon.
  - extracted: Provision a new machine with NVIDIA GPU and drivers 2.
  - extracted: Install go livepeer 3.
