...
- `v2/gateways/guides/node-pipelines/ai-pipelines.mdx`
  ...
  - extracted: `<Card title="Source reference: AISessionManager" icon="github" href="https://github.com/livepeer/go livepeer/blob/master/server/ai http.go" horizontal arrow>` go livepeer/server/ai http.go — AI vs Video Comparison: Aspect / Video Transcoding / AI Inference. Payment model: On chain PM tickets (ETH deposit required) vs Off chain (no ETH required). Ingest format: RTMP stream or HTTP segment push vs Discrete HTTP POST per job. Output: Streaming HLS manifest vs JSON response (images, text, audio). Session type: Long running streaming session vs Short request/response. Orchestrator selection: Price + latency + segment history vs Pipeline capability + model availability + price. GPU requirement on orchestrator: Optional (CPU mode exists) vs Required. Available Pipelines: Livepeer AI inference routes across three integration patterns.
- `v2/gateways/guides/node-pipelines/byoc-pipelines.mdx`
  - claim families: none
