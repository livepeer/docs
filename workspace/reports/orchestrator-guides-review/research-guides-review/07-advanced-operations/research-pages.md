...
- `v2/orchestrators/guides/advanced-operations/gateway-orchestrator-interface.mdx`
  ...
  - extracted: This pattern suits operators with a single high capacity machine who want to eliminate network latency between gateway and orchestrator.
  - extracted: Your orchestrator competes on price and stake like any other.
- `v2/orchestrators/guides/advanced-operations/gateway-relationships.mdx`
  - claim families: none
  - extracted: Understanding that pipeline is what separates operators who receive steady work from those who are technically online but still idle.
...
- `v2/orchestrators/guides/advanced-operations/pool-operators.mdx`
  ...
  - extracted: Delegators and Explorer see only your orchestrator identity, and all stake, protocol reputation, and on chain fees flow through that address.
...
- `v2/orchestrators/guides/advanced-operations/scale-operations.mdx`
  - claim families: none
  - extracted: You are in fleet territory when these conditions start to apply: Your workload requires more GPU capacity than fits in one machine You need geographic distribution for latency sensitive AI workloads You want to separate concerns — one node for reward calling, one for ticket redemption, separate GPU workers You are operating at data centre scale with SLA commitments to gateway operators Start with a single node unless one of these conditions already applies.
  ...
  - extracted: The same principle separates reward calling from workload processing, but the split is implemented entirely within go livepeer instead of through OrchestratorSiphon.
