...
import { DoubleIconLink, LinkIcon } from '/snippets/components/elements/links/Links.jsx'
...
const explorerBase = {
  arbitrumOne: contractAddresses.meta?.explorerUrls?.arbiscanAddress || 'https://arbiscan.io/address/',
  ethereumMainnet: contractAddresses.meta?.explorerUrls?.etherscanAddress || 'https://etherscan.io/address/',
}
...
export const activeTableColumnWidths = {
  Type: '12%',
  Chain: '28%',
}

export const proxyTableColumnWidths = {
  Name: '16%',
  Proxy: '5%',
  'Proxy Address': '26%',
  Chain: '22%',
...
export const publishedTableColumnWidths = {
  Chain: '30%',
  Status: '22%',
}

export const activeTableItems = [
  ...(contractAddresses.arbitrumOne?.active || contractAddresses.arbitrumOne?.current || []).map((entry) => ({
    Name: entry.name,
    Address: entry.address,
    _addressHref: entry.blockchainHref || `${explorerBase.arbitrumOne}${entry.address}`,
    Type: entry.type || entry.deploymentKind || 'standalone',
    Category: entry.category,
    Chain: chainLabels.arbitrumOne,
  })),
  ...(contractAddresses.ethereumMainnet?.active || contractAddresses.ethereumMainnet?.current || []).map((entry) => ({
    Name: entry.name,
    Address: entry.address,
    _addressHref: entry.blockchainHref || `${explorerBase.ethereumMainnet}${entry.address}`,
    Type: entry.type || entry.deploymentKind || 'standalone',
    Category: entry.category,
    Chain: chainLabels.ethereumMainnet,
  })),
].filter((entry) => entry.Type === 'standalone')

export const activeProxyBaseItems = [
  ...(contractAddresses.arbitrumOne?.active || contractAddresses.arbitrumOne?.current || []).map((entry) => ({
    ...entry,
    _chain: 'arbitrumOne',
  })),
  ...(contractAddresses.ethereumMainnet?.active || contractAddresses.ethereumMainnet?.current || []).map((entry) => ({
    ...entry,
    _chain: 'ethereumMainnet',
  })),
].filter((entry) => (entry.type || entry.deploymentKind) === 'proxy')

const toPublishedRows = (entries, chain, status) =>
  (entries || []).map((entry) => ({
    Name: entry.name,
    Address: entry.address,
    _addressHref: entry.blockchainHref || `${explorerBase[chain]}${entry.address}`,
    Type: entry.type || entry.deploymentKind || 'standalone',
    Status: status,
    Category: entry.category,
    Chain: chainLabels[chain],
  }))

export const publishedNonActiveItems = [
  ...toPublishedRows(contractAddresses.arbitrumOne?.paused, 'arbitrumOne', 'Paused'),
  ...toPublishedRows(contractAddresses.ethereumMainnet?.paused, 'ethereumMainnet', 'Paused'),
  ...toPublishedRows(contractAddresses.arbitrumOne?.migration_residual, 'arbitrumOne', 'Migration residual'),
  ...toPublishedRows(contractAddresses.ethereumMainnet?.migration_residual, 'ethereumMainnet', 'Migration residual'),
  ...toPublishedRows(contractAddresses.arbitrumOne?.legacy_operational, 'arbitrumOne', 'Legacy operational'),
  ...toPublishedRows(contractAddresses.ethereumMainnet?.legacy_operational, 'ethereumMainnet', 'Legacy operational'),
]
...
{/* Approved style state: active table keeps Type narrow and Chain wide. */}
<SearchTable
  TableComponent={DynamicTable}
  headerList={["Name", "Address", "Type", "Chain"]}
  itemsList={activeTableItems}
  columnWidths={activeTableColumnWidths}
  monospaceColumns={[]}
...
{/* Approved style state: six columns, narrow Proxy/Target badge columns, styled address cells. */}
<SearchTable
  TableComponent={DynamicTable}
  headerList={["Name", "Proxy", "Proxy Address", "Chain", "Target", "Target Address"]}
  itemsList={activeProxyBaseItems.map((proxy) => {
    const target = (contractAddresses[proxy._chain]?.currentImplementations || []).find((entry) => entry?.name === proxy.name)
    return {
      Name: proxy.name,
      Proxy: 'Proxy',
      'Proxy Address': <>
        <CopyText text={proxy.address} />
        {proxy.blockchainHref ? <> <LinkIcon href={proxy.blockchainHref} color="var(--accent)" /></> : null}
      </>,
      Chain: chainLabels[proxy._chain],
      Target: target?.address ? 'Target' : '—',
      'Target Address': target?.address ? <>
        <CopyText text={target.address} />
        <LinkIcon href={target.blockchainHref || `${explorerBase[proxy._chain]}${target.address}`} color="var(--accent)" />
      </> : '—',
      Category: proxy.category,
    }
  })}
  columnWidths={proxyTableColumnWidths}
  monospaceColumns={[]}
  ...
  separatorColumn="Category"
  boldFirstColumn={true}
  columnVariant={{ Proxy: "badge", Chain: "textIcon", Target: "badge" }}
  categoryBadges={[
...
<SearchTable
  ...
  itemsList={publishedNonActiveItems}
  columnWidths={publishedTableColumnWidths}
  monospaceColumns={[]}
