...
function buildHistoricalArtifacts({ chain, entries = [], currentImplementations = [], previousChainPayload = null }) {
  ...
  previousSeriesMap.forEach((series) => {
    series.entries.forEach((entry) => {
      entry.isCurrent = false;
      entry.status = null;
      entry.replacedBy = null;
  ...
  buildCurrentSeriesCandidates(entries, currentImplementations).forEach((candidate) => {
    const key = seriesGroupKey(candidate);
    ...
    const matchingEntries = series.entries.filter((entry) => entry.chain === candidate.chain && entry.type === candidate.type);
    const existing = matchingEntries.find((entry) => addressesMatch(entry.address, candidate.address));
    if (existing) {
      existing.isCurrent = true;
      existing.version = existing.version || candidate.version || inferNextSeriesVersion(matchingEntries);
      existing.blockchainHref = existing.blockchainHref || candidate.blockchainHref;
      return;
    }
    series.entries.push({
      ...candidate,
      version: inferNextSeriesVersion(matchingEntries, candidate.version),
      isCurrent: true,
    });
  });
...
function writeDataFiles(data, blockchainPageData, governorSha, catalog, dryRun = false) {
  const now = new Date();
  const nowISO = now.toISOString();
  const nowFormatted = now.toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" });
  ...
  fs.writeFileSync(BLOCKCHAIN_PAGE_DATA_PATH, blockchainJsx);
  fs.writeFileSync(BLOCKCHAIN_PAGE_DATA_JSON_PATH, JSON.stringify(blockchainJsonPayload, null, 2));
...
async function runContractsPipeline(options = {}) {
  ...
  const writtenPayload = writeDataFiles(payload, blockchainPageData, sha, catalog, dryRun);
