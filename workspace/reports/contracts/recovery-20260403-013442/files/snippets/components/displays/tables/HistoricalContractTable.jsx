...
export const HistoricalContractTable = ({ category = "core", sourceData = {} }) => {
  ...
  const visibleSeries = [...byName.values()]
    .map((series) => {
      const historicalEntries = series.entries.filter((entry) => !entry.isCurrent)
                ...
                <tbody>
                  {entries.map((c, i) => (
                    <tr key={i}>
                      ...
                      <td style={cp}><span style={hs}><Icon icon="circle" color="grey" size={8} /> {c.status || (c.replacedBy ? `Replaced by ${c.replacedBy}` : "Deprecated")}</span></td>
                    </tr>
                  ))}
                </tbody>
              </table>
