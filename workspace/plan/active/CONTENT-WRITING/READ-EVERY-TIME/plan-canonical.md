...
- v5 learnings — Orchestrators ✅ created (file confirmed at `Prompts/Prompts-By-Phase/1-audience-design/testing/Tabs/orchestrators/learnings/v5.md`)
- Content scans — Gateways ✅ · About ✅ · Delegators ✅ · Developers ✅ (all four completed 2026-03-23; `context-packs/[tab]-content-scan.md` files confirmed on disk)
- **D-NAV-01** — Navigation disambiguation page locked as cross-tab pattern (all multi-path tabs). Recorded in `decisions/decision-registry.md`. ✅ LOCKED 2026-03-23
- **Confirmed path fact**: v2 path for the Delegators/LPT tab is `v2/lpt/` — not `v2/delegators/` or `v2/lp-token/`. Confirmed by filesystem. All agents reading or writing Delegators v2 content must use this path.
...
- Full v2 content reads still missing: `context-packs/orchestrators-full-content.md`, `context-packs/about-full-content.md`, `context-packs/delegators-full-content.md` — agents running these may have failed. Needed for Phase 1b. Verify and re-spawn if not returned.
- Content scan — Orchestrators: `context-packs/orchestrators-content-scan.md` — still to run
...
**Already locked** (not listed in the open items table below):
- **D-NAV-01** — Navigation disambiguation page is a locked cross-tab pattern for all multi-path tabs. LOCKED 2026-03-23. Full record in `decisions/decision-registry.md`.
- **v2/lpt/ path** — Confirmed v2 path for Delegators/LPT tab is `v2/lpt/`. Not `v2/delegators/` or `v2/lp-token/`. Confirmed by filesystem.
...
> **Process authority**: All operational rules — Confirm-Before-Spawn, Per-Tab Flow sequence, session start protocol, gate definitions, and agent prohibitions — are in `PROJECT-MANAGEMENT-CANONICAL.md`. That file governs. The sections below describe phase-level context (what each phase is for, its inputs/outputs, agent types) but do not duplicate operational rules. If you find a conflict between this file and `PROJECT-MANAGEMENT-CANONICAL.md`, `PROJECT-MANAGEMENT-CANONICAL.md` wins.

---

### Per-Tab Flow

**The authoritative 20-step per-tab sequence is in `PROJECT-MANAGEMENT-CANONICAL.md` Part 5.** Do not execute from memory or from the compressed summary below — read Part 5 before running any per-tab phase.

**Summary for orientation only** (not a substitute for PM-CANONICAL Part 5):
- Steps 1–4: Human IA review → Phase 1b persona check → human iteration decision → human locks IA
- Steps 5–6: Check B validation (agent) + deep glossary research pass (agent, parallel after lock)
- Steps 7–9: Content scan (agent) → human checkpoint on scan
- Step 10: Phase 2.5 IA-mapped research pack (agent, after IA locked)
- Steps 11–12: IA Audit / Structure Audit (agent) → human approves tab-map + persona journey smoke-test
- Steps 13–16: Terminology lock → Content Audit → human approves reconsolidation → Reconsolidation execution
- Steps 17–20: Content Pass → Naming Audit (Phase 8, before Phase 7) → Veracity Pass (Phase 7) → Layout Pass

**Phase 8 runs BEFORE Phase 7.** Naming changes section titles; veracity markers placed before naming is final become orphaned. See PM-CANONICAL Steps 18–19.
...
> **See `PROJECT-MANAGEMENT-CANONICAL.md` Part 2 (Agent Rules) for the full mandatory pre-phase checklist.** Key points:

1. Read `ai-rules-guides.md` — governs how all agents work in this repo
2. Read the phase's `pack-guide.md` — pre-flight steps, running order, known failure modes
3. Re-read `decisions/decision-registry.md` for any decisions affecting this phase
4. Verify pre-conditions explicitly before executing — if a pre-condition is false, correct the plan before proceeding
5. Every agent derives its own dependency list from plan-canonical.md — does not trust the prompt author's stated list

### Agent types — use the right one for the work

**Design-thinking / product-thinking agents** — use when the phase involves open questions, structural decisions, or strategic framing.
...
**Specialist tools**: `glossary-terminology-generation`, `docs-json-navigation-maintenance`, `component-library-guided-authoring`, `docs-source-verification`

---

## Prompt Testing Protocol

> **Full protocol in `prompt-testing-protocol.md`.** Summary:

1. **1 internal review agent** — runs Check A (best practices) + Check B (canonical design alignment) + Check C (generality across tabs) in one pass
2. **3 agent test runs** — at least one multi-audience tab (About or Community) included
3. **Collate learnings** — severity: `CRITICAL` / `HIGH` / `MEDIUM` / `INFO`
4. **Human approval** — before any production use
5. **Version learnings file** — per version, per tab

---

## Project Management

> **All PM rules, gate definitions, and operational protocols are in `PROJECT-MANAGEMENT-CANONICAL.md`.** The files below are live runtime documents — read them, update them, do not duplicate their rules here.

- **Decision registry**: `decisions/decision-registry.md` — all locked structural decisions. No phase may consume a decision not in this registry.
- **Tab status board**: `decisions/tab-status.md` — per-tab gate tracking (IA Approved / Terminology Locked / Content Scan Done / Structure Approved / Content Pass Open / Veracity Done / Layout Done). Live state is there, not here.
- **Blocking items**: `decisions/blocking-items.md` — runtime, populated by humans during phase reviews. Not pre-populated from AI runs.
- **Feedback routing map**: `decisions/feedback-routing-map.md` — required for Content Pass Open gate.

> Note: "create this file" notes that appeared here previously are removed — all these files now exist.
...
**Already locked (recorded in decision-registry.md)**:
- **D-NAV-01** — Navigation disambiguation page pattern locked 2026-03-23 for all multi-path tabs
- **v2/lpt/ path** — Confirmed v2 path for Delegators/LPT tab is `v2/lpt/` (not `v2/delegators/` or `v2/lp-token/`) — confirmed on disk 2026-03-23

**Decisions still needed, recorded in decision-registry.md**:
...
- **v1 vs v2 canonical path**: Confirm `v2/[tab]/` is the canonical source for all Phase 2+ work. For Delegators this is `v2/lpt/` (already confirmed). For remaining tabs — confirm before Phase 3 runs.
...
**lp-token and About**: canonical collation outputs ✅ produced (completed 2026-03-23). Both tabs have collated output files. Check B validation and human review still required before either tab advances to Phase 2.
...
**v5 learnings status**:
- Orchestrators: ✅ created (`Prompts/Prompts-By-Phase/1-audience-design/testing/Tabs/orchestrators/learnings/v5.md` — confirmed on disk)
- Gateways, Developers, LPT, About: ❌ missing — tracked in ❌ Missing section
...
> **Artefact distinction — do not confuse**: The input to Phase 1b is the raw inventory pack (`[tab]-research-pack.md`, secondary tier). This is NOT the Phase 2.5 IA-mapped research pack (`[tab]-research-pack-v2.md`). The raw packs were produced without IA context — that is precisely why they are used here. The IA-mapped packs are produced in Phase 2.5, after the IA is locked.
...
**Note**: For tabs where raw inventory is thin (Delegators — v2/lpt/ path, 6 files plus 3 misrouted Studio docs flagged), Phase 1b may not surface new personas. Run it anyway — the null result is a valid confirmation.

---

### Phase 2 — Content Scan

**Current status**: ✅ Complete for Gateways, About, Delegators, Developers. Orchestrators: ⬜ still to run. All four completed scans require human review before Phase 3 runs for those tabs. **Note**: Delegators scan reads `v2/lpt/` (confirmed v2 path).
...
**This is NOT the raw inventory pack and is NOT the Phase 2 content scan**. Three distinct artefacts exist per tab — do not confuse them:
- `[tab]-research-pack.md` — raw secondary inventory (Phase 1b input, already produced for all 5 tabs)
- `[tab]-content-scan.md` — Phase 2 structured page inventory (produced by content-scan.md prompt)
- `[tab]-research-pack-v2.md` — THIS output: IA-mapped research pack (Phase 2.5, built from scratch after IA locked)

The raw inventory packs (`[tab]-research-pack.md`) are useful as raw material but are not the final research packs. This phase builds from scratch using the approved IA as primary structure.
...
5. Human reviews every WRITE output before it moves to Phase 8 (naming — Phase 8 runs before Phase 7)
...
**ORDERING NOTE**: Phase 8 (naming) must execute BEFORE Phase 7 (veracity). If naming changes section titles after veracity markers are placed, the markers become orphaned. Correct ordering: Phase 6 → Phase 8 (naming) → Phase 7 (veracity) → Phase 10 (layout). This is the authoritative ordering per `PROJECT-MANAGEMENT-CANONICAL.md` Steps 18–19. The phase section numbering in this document (Phase 7 heading before Phase 8 heading) does not reflect execution order — Steps 18–19 in PM-CANONICAL are the execution authority.
...
### Phase 8 — Naming Audit & Glossary (BEFORE Phase 7 — see PM-CANONICAL Step 18)

**Execution order**: Phase 8 runs BEFORE Phase 7. This is Step 18 (naming) → Step 19 (veracity) in `PROJECT-MANAGEMENT-CANONICAL.md` Part 5. Phase section numbering in this document does not reflect execution order.
...
| PM | Create decisions/terminology-tracking.md stub | ✅ Done |
| PM | Create PROJECT-MANAGEMENT-CANONICAL.md | ✅ Done |
| PM | Create DEPENDENCY-TREE.md | ✅ Done |
| PM | Lock D-NAV-01 (navigation disambiguation pattern — all multi-path tabs) | ✅ Done (2026-03-23) |
| PM | Confirm v2/lpt/ as canonical path for Delegators tab | ✅ Done (confirmed on disk 2026-03-23) |
...
| 1 | Run retroactive Check B validation — Developers (before Phase 2 unlocks) | ⬜ |
| 1 | Run retroactive Check B validation — About (before Phase 2 unlocks) | ⬜ |
| 1 | Run retroactive Check B validation — Delegators (before Phase 2 unlocks) | ⬜ |
...
| 2 | Run content scan — About | ✅ Done (completed 2026-03-23) |
| 2 | Run content scan — Delegators | ✅ Done (completed 2026-03-23) |
| 2 | Run content scan — Developers | ✅ Done (completed 2026-03-23) |
