...
The current `level-1-tab-map.md` tries to do both and ends up doing neither well.

- Prompt B (audit) is usable as-is for reviewing an existing tab against a known structure.
- Prompt A (design) does not exist yet. It needs to be built.
