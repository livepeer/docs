...
> **Status**: Active — Tasks 1–15, 17, 18, 19 complete. Remaining: T16 (test pass in progress), T20, T21.
...
- 12.8 Run existing test suite (`operations/tests/run-all.js`)
...
- 16.4 Run test suite (`operations/tests/run-all.js`)
...
### ~~Task 19 — Composable templates~~ COMPLETED (2026-03-21)

> Commits: `9ef6277f` (composables), `346bb6a0` (documentation)
...
**Deferred (explicitly out of scope for T19):**
- `component-layout-decisions.mdx` update → CONTENT-STRUCTURE Phase 5.1
- `@contentAffinity` enforcement → add to `--strict` in CONTENT-STRUCTURE Phase 5.1
...
## Recommended Execution Order (as of 2026-03-21)

### This session — close out component governance

| Step | Task | Effort | Notes |
|---|---|---|---|
| 1 | **T16** — Run full test pass | Fast | `node operations/tests/run-all.js`, confirm zero new errors |
| 2 | **T19.1** — Checkpoint: approve composable shortlist | Interactive | Review `composables-research.md` Tier 1–2 candidates |
| 3 | **T19.2–3** — Build Tier 1+2 composables | Medium | 4+2 `.mdx` files in `snippets/composables/` |
| 4 | **T19.4** — Documentation + `@contentAffinity` | Medium | README, governance.mdx composables section, component-layout-decisions.mdx update |
| 5 | **T19.5** — Checkpoint: review composables + docs | Interactive | Human review |
| 6 | **T20** — Final testing + cleanup | Medium | Full test pass; confirm clean |
| 7 | **T21** — Close plan | Fast | Move to completed/ |

### Parallel track — AI discoverability quick wins (no scripts)

| Step | Task | Effort | Notes |
|---|---|---|---|
| A | **CDA-1** — Registry captures `@aiDiscoverability` | Low | Single field in `generate-component-registry.js` |
| B | **CDA-2** — Companion JSON schema | Low | Write `docs-guide/catalog/ai-companion-schema.json` |
| C | **CDA-3** — Manual Tier 1 snapshots | Low | CoinGecko → `snippets/data/snapshots/coingecko-livepeer.json` |
| D | **CDA-4** — Manual Tier 2 props extraction | Low | Grep SearchTable/ShowcaseCards MDX pages → write `-data.json` companions |

### Unlock content writing

| Step | Task | Effort | Notes |
|---|---|---|---|
| E | **CONTENT-WRITING Step 6** — Lock industry/niche | Interactive | DRAFT since 2026-03-19; everything downstream blocked |
| F | **CONTENT-WRITING Step 8** — IA blueprint per tab | Large | Blocked on Step 6; defines what pages must exist per audience tab |

### Other active work (whenever ready)

| Plan | Next action |
|---|---|
| **SOLUTIONS-SOCIAL-DATA** | Review Phase 2 at `workspace/plan/active/SOLUTIONS-SOCIAL-DATA/plan.md` |
| **SCRIPT-GOVERNANCE** | T13b (autogenerated file governance), T14 (operations/ move) |
| **REPO-STRUCTURE-GOV** | Phase 0 low-risk items |
| **PERFORMANCE-OPT** | GIF replacement, lazy iframe loading |
| **CANONICAL-TRUTH-GUIDES** | After T21 closes component governance |
