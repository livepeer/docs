...
|---|---|---|
| `@aiDiscoverability` field values | `workspace/plan/active/AI-DISCOVERABILITY/` (CDA tasks) | The `snapshot` / `props-extracted` / `none` enum and companion file conventions are defined by the AI Discoverability framework. Any extension of the enum must be proposed there first. |
...
| `component-layout-decisions.mdx` page types | `workspace/plan/active/CONTENT-WRITING/` (Content Framework) | The 13 page types in the matrix are locked by the Content Framework (`pagePurpose.md`, `information-type.md`). Any new page types added to v2 must go through the Content Framework first; this doc is downstream. |
