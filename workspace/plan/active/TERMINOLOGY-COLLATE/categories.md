...
## Canonical Tagging Taxonomy (Human-Approved)

**Date approved**: 2026-03-20
**Rule**: Tags are an array. If a term fits multiple sub-categories, tag it with ALL that apply.

### 7 Domains, 35 Sub-categories

| Domain | Sub-category | Scope |
|---|---|---|
| **livepeer** | `livepeer:protocol` | Core protocol mechanics — Round, Active set, Segment, Reward call, Slashing, Capability |
| | `livepeer:role` | Network actors — Orchestrator, Gateway, Delegator, Transcoder, Worker, Verifier |
| | `livepeer:product` | Products and apps — Livepeer Studio, Daydream, Streamplace, Embody, ComfyStream, Livepeer Cloud |
| | `livepeer:upgrade` | Protocol upgrade names — Snowmelt, Tributary, Streamflow, Confluent, Cascade, Delta |
| | `livepeer:contract` | Smart contracts — BondingManager, Minter, RoundsManager, Governor, TicketBroker, ServiceRegistry, AIServiceRegistry |
| | `livepeer:config` | CLI flags, config files, node settings — aiModels.json, aiWorker, orchSecret, pricePerUnit, MaxPrice, Service URI |
| | `livepeer:deployment` | Deployment types and modes — Solo operator, Pool, O-T split, Siphon, Dual mode, On-chain/Off-chain gateway |
| | `livepeer:sdk` | SDKs and libraries — livepeer-js, livepeer-ai-js, livepeer-go, @livepeer/react, PyTrickle, Trickle Protocol |
| | `livepeer:entity` | Organisations — Livepeer Foundation, Livepeer Inc, SPE, GovWorks, LiveInfra, StableLab |
| | `livepeer:tool` | Ecosystem tools — Livepeer Explorer, awesome-livepeer, Livepeer.Tools, Stream Tester, Titan Node |
| **video** | `video:protocol` | Streaming protocols — RTMP, RTMPS, HLS, SRT, WebRTC, WHIP, WHEP, DASH |
| | `video:encoding` | Encoding concepts — Codec, H.264, HEVC, VP9, AV1, Bitrate, Resolution, CRF, CBR, B-frames, Keyframe, Frame rate |
| | `video:processing` | Processing operations — Transcoding, Ingest, Delivery, Segmentation, Rendition, Encoding ladder, Quality Ladder |
| | `video:playback` | Playback concepts — Latency, VOD, Livestream, Streaming, ABR, TTFF, Rebuffer ratio, Thumbnail, Player |
| | `video:studio` | Livepeer Studio objects — Stream, Session, Asset, Clip, Playback ID, Stream key, Recording, Multistream, Room, Project |
| **ai** | `ai:pipeline` | Pipeline types — Text-to-Image, Image-to-Image, Image-to-Video, Live-video-to-video, Text-to-Speech, Audio-to-text, Segmentation, Upscaling |
| | `ai:model` | Specific models — Stable Diffusion, SDXL, SVD, Whisper, BLIP, SAM 2, ControlNet, LoRA, StreamDiffusion, Ollama |
| | `ai:concept` | General AI concepts — Inference, Model, Diffusion, Embedding, LLM, Transformer, Multimodal, Batch AI, Real-time AI, Warm model, Cold model |
| | `ai:framework` | AI frameworks and tools — TensorRT, PyTorch, CUDA, ComfyUI |
| | `ai:application` | AI use cases — World model, Avatar, Digital twin, NPC, Style transfer, Generative video, Synthetic data, Autonomous agent |
| **web3** | `web3:chain` | Chains and layers — Ethereum, Arbitrum, Layer 1, Layer 2, Mainnet, Testnet |
| | `web3:token` | Tokens and standards — LPT, ETH, ARB, ERC-20, NFT, Wei |
| | `web3:tokenomics` | Token economics — Bonding, Unbonding, Delegation, Stake, Bonding rate, Target bonding rate, Inflation adjustment, Tokenomics, Staking, Network effects, TAM |
| | `web3:concept` | Blockchain primitives — Smart contract, DAO, On-chain, Off-chain, Gas, Block, Transaction, Proof-of-stake, Rollups, Bridging, DePIN, Finality, Merkle Mine, ICO |
| | `web3:governance` | Governance primitives — Quorum, Timelock, Stake-weighted voting, Atomic execution, Proposer bond |
| | `web3:identity` | Addressing and identity — Ethereum Address, Wallet, Private key, Public key, Block Timestamps |
| **economic** | `economic:payment` | Payment mechanisms — Probabilistic micropayments, Payment ticket, Winning ticket, Ticket redemption, Clearinghouse, Settlement, Deposit, Reserve, Face value, Win probability |
| | `economic:pricing` | Pricing models — Pay-per-pixel, Per-request pricing, pricePerUnit, pricePerCapability, USD pricing, Price feed, Service margin, Arbitrage |
| | `economic:reward` | Rewards and staking economics — Inflation, Inflationary rewards, Reward cut, Fee cut, Fee share, Yield, Commission rate, Dilution, Vesting, Genesis supply |
| | `economic:treasury` | Treasury mechanics — Treasury, Treasury allocation, Community treasury, Treasury reward cut rate, Grant, Bounty, Quadratic funding, Retroactive funding, Fee switch |
| | `economic:business` | Business and investment — TAM, MoC, Network effects, ICO, Market opportunity |
| **technical** | `technical:infra` | Infrastructure — GPU, VRAM, NVENC, NVDEC, Docker, Container, Node, Bandwidth, CDN, DNS, Load balancer, Kubernetes, Edge compute |
| | `technical:protocol` | Technical protocols — HTTP, HTTPS, JSON, REST, GraphQL, WebSocket, gRPC, OAuth, JWT, TLS |
| | `technical:dev` | Developer tools — SDK, CLI, API, Endpoint, Webhook, API key, Bearer token, Solidity, CORS |
| | `technical:security` | Security concepts — AES-CBC, C2PA, Provenance, Remote signer, Signing key, Access control, Encrypted asset |
| | `technical:social` | Social protocols — AT Protocol, Fediverse |
| **operational** | `operational:governance` | Governance processes — LIP, Pre-Proposal, Voting delay, Voting period, Passing threshold, Vote detachment, RFP |
| | `operational:community` | Community programs — Treasury Talk, Water Cooler, Dev Call, Live Pioneers, KYO, Ambassador Programme, Forums |
| | `operational:monitoring` | Monitoring and metrics — Prometheus, Grafana, Loki, Dune, Performance score, Throughput, Transcode fail rate, Uptime, Smoke test, Failover |
| | `operational:process` | Operational concepts — Scaling, Workload, Overhead, Onboarding, KYC, Conventional Commits |

---

## Legacy Category Overview (from initial harvest)
