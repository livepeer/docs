...
- `v2/orchestrators/guides/deployment-details/requirements.mdx`
  - claim families: `orch-bandwidth-per-stream-planning`, `orch-batch-ai-vram-threshold`, `orch-business-case-cost-viability`, `orch-session-limit-default`
...
- `v2/orchestrators/setup/configure.mdx`
  ...
  - extracted: Essential Flags These flags must be set every time go livepeer starts as an Orchestrator.
  - extracted: Options: production network local or dev only, no on chain participation Points go livepeer at an Arbitrum One RPC endpoint to read and write on chain state Orchestrator registration, reward calls, ticket redemption.
  - extracted: This must be an Arbitrum One endpoint, not an Ethereum L1 endpoint.
  - extracted: This account holds staked LPT, receives ETH fee payments, and signs on chain transactions.
- `v2/orchestrators/setup/rcs-requirements.mdx`
