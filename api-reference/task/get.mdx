...
```bash cURL
curl --request GET \
  --url https://livepeer.studio/api/task/{id} \
...
func main() {
	...
	res, err := client.Task.Get(ctx, id)
...
req = Operations::GetTaskRequest.new(
  path_params=Operations::GetTaskRequest.new(
    task_id="08eeeda4-681f-46e5-a760-63d6cac91d91",
  ),
)

res = client.task.get(req)
