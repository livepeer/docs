...
<Accordion title="See Workflow Verification Information">
  This page reads the generated contracts registry `contractAddressesData.jsx`. That file is the canonical persisted contracts dataset in this repo, and it is rebuilt by the contracts workflow rather than maintained by hand.

  <Steps>
    <Step title="Workflow Trigger">
      [`update-contract-addresses.yml`](https://github.com/livepeer/docs/blob/main/.github/workflows/update-contract-addresses.yml) runs daily at `02:00 UTC` (`0 2 * * *`), supports manual dispatch with `dry_run`, `skip_verify`, and `use_test_branch`, and can also be started by `repository_dispatch` events from [`livepeer/protocol`](https://github.com/livepeer/protocol), [`livepeer/arbitrum-lpt-bridge`](https://github.com/livepeer/arbitrum-lpt-bridge), [`livepeer/go-livepeer`](https://github.com/livepeer/go-livepeer), and [`livepeer/governor-scripts`](https://github.com/livepeer/governor-scripts).
    </Step>

    <Step title="Canonical source inputs">
      ...
      - watched repo metadata, default branches, branch inventories, commits, and file contents via the GitHub REST API endpoints:
        - `/repos/{repo}`
    ...
    <Step title="Resolve Contract Set">
    ...
    <Step title="Verify Published Addresses">
    ...
    <Step title="Run Verification Pass">
  ...
  **Current limit:** the live pipeline verifies the proof catalog aggressively, but it still resolves the known catalog defined in `spec.js` rather than discovering arbitrary new contract families directly from repo diffs.
</Accordion>
