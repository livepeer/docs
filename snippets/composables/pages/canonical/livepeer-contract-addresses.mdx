...
<Accordion title="See Workflow Verification Information">
  The contracts registry on this page is generated by the GitHub Actions workflow [`update-contract-addresses.yml`](https://github.com/livepeer/docs/blob/main/.github/workflows/update-contract-addresses.yml). It is not maintained by hand.

  <Steps>
    <Step title="1. Trigger">
      The main workflow runs daily at `02:00 UTC` (`0 2 * * *`), supports manual dispatch with `dry_run`, `skip_verify`, and `use_test_branch`, and can also be started by `repository_dispatch` events with these types: `governor-scripts-update`, `protocol-update`, `bridge-update`, and `go-livepeer-update`.
    </Step>

    <Step title="2. Load sources">
      `.github/scripts/fetch-contract-addresses.js` starts the pipeline, which loads the proof catalog, the governor addresses manifest, the previous successful contracts payload, and the previous branch-watch snapshot.
    </Step>

    <Step title="3. Resolve contracts">
      The live pipeline resolves the contract families currently defined in its proof catalog from live controller reads, deployment artefacts, runtime checks, and repo-pinned provenance.
    </Step>

    <Step title="4. Verify and enrich">
      It verifies bytecode on Arbitrum One and Ethereum Mainnet, enriches proxy and controller metadata, and rebuilds Arbitrum historical entries from controller `SetContractInfo` logs.
    </Step>

    <Step title="5. Publish canonical data">
      If validation passes, the pipeline writes the canonical generated registry `snippets/data/contract-addresses/contractAddressesData.jsx`, and the JSON and page-facing companion outputs are derived from that dataset.
    </Step>

    <Step title="6. Fail safely">
      If validation fails, the workflow writes health checks and anomaly artefacts, uploads them, creates or updates a GitHub incident issue, and does not publish replacement contract data.
    </Step>

    <Step title="7. Shadow verification">
      `update-contract-addresses-shadow.yml` reruns the verification path daily at `02:30 UTC` (`30 2 * * *`) in check-only mode and never publishes changes.
    </Step>
  </Steps>

  **Current limit:** watched repos protect provenance and branch changes, but the live pipeline still resolves the known proof catalog rather than discovering arbitrary new contract families directly from repo diffs.
</Accordion>
