...
<Update label="DEMON: Live LoRA Support" tags={["Features", "Fixes", "DEMON"]} rss={{ title: "Daydream DEMON: Live LoRA Support", description: "SA3 backend gains LoRA support with TensorRT refit, running pods can fetch and register new LoRAs mid-session, and st..." }} description={<Subtitle variant="changelog">August 2026</Subtitle>}>
  ## DEMON: Live LoRA Support

  <CustomCardTitle variant="tab" icon="user-robot" title="_AI Summary_" />

  #### New Features

  - **SA3 LoRA Support** – The Stable Audio 3 backend now supports LoRAs with TensorRT refit, so custom styles apply without rebuilding an engine. Try it on the hosted demo at [music.daydream.live](https://music.daydream.live).
  - **Live LoRA Loading** – A running session can fetch and register a new LoRA on demand, so a style trained while you are playing is usable in the same session without a restart.

  #### Bug Fixes

  - **Per-Session Style Isolation** – Fetched LoRAs are now isolated per session and wiped on session close, so styles you use never leak to the next user of the same pod.
  - **SA3 LoRA Registration** – Fixed a path that prevented runtime-fetched LoRAs from registering on SA3 sessions.
