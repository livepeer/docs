...
purpose: changelog
audience: builder
lastVerified: 2026-05-11T00:00:00.000Z
---
...
<Update label="Scope: v0.2.4" tags={["Features", "Fixes", "Scope"]} rss={{ title: "Daydream Scope v0.2.4", description: "This release introduces the Node Abstraction, LTX-2 improvements, and a sweep of UX polish and bug fixes." }} description={<Subtitle variant="changelog">April 2026</Subtitle>}>
  ...
  - **Node Abstraction** – Ship any custom node for Scope as a plugin. Sample plugins include a YouTube input source and a local LLM Prompt Enhancer.
  - **LTX-2 improvements** – Real-time pacing controls, roughly 18× faster model loading, faster prompt changes, and Ampere GPU compatibility.
  - **Expanded OSC coverage** – Any node parameter can now be exposed via OSC with per-parameter opt-in.

  #### Updates

  - **Graph editor UX** – Edge unattach, quick-connect drag menu, in-node audio playback, a "?" cheat sheet, and install hints for unavailable integrations (NDI / Spout / Syphon).
  - **Recording reliability** – Replaced media recorder, preserved incoming timestamps, added dedicated record-node queues, and tightened A/V start sync.

  #### Bug Fixes

  - **VACE reference propagation** – Fixed reference image handling in VACE pipelines.
  - **Spout pixel format** – Resolved a pixel-format flip issue on Spout output.
  - **LoRA error clarity** – Friendlier error message when loading LoRAs from Hugging Face `/blob/` URLs.
  - **Workflow start** – Source-less workflows can now start cleanly.
...
<Update label="Scope: v0.2.3" tags={["Features", "Fixes", "Scope"]} rss={{ title: "Daydream Scope v0.2.3", description: "This release introduces Synced Audio + Video, the move to Livepeer Network as the default cloud backend, and Cloud Billing." }} description={<Subtitle variant="changelog">April 2026</Subtitle>}>
  ## Scope: v0.2.3
  ...
  - **Synced audio + video** – Timestamps now ride with every video and audio packet end-to-end, with cumulative-drift pacing that keeps output steady over long runs.
  - **Livepeer Network cloud default** – Cloud mode now runs on the Livepeer Network by default, with audio and multi-track support.
  - **Cloud billing** – Credit-based billing with subscription management is now available for cloud users.

  #### Updates

  - **Graph mode redesign** – Refreshed visuals for the graph mode editor and a brighter background grid.

  #### Bug Fixes

  - **Headless recording** – Resolved recording issues in headless mode.
  - **Safari stats** – Fixed sink statistics reporting on Safari.
  - **Syphon reliability** – Addressed flip and reload bugs on Syphon sources.
  - **macOS sink fullscreen** – Fixed fullscreen behavior on the macOS sink.
...
<Update label="Scope: v0.2.2" tags={["Features", "Fixes", "Scope"]} rss={{ title: "Daydream Scope v0.2.2", description: "This release introduces Multi Source / Multi Sink graphs, an Audio In node, and a number of bug fixes across graph mode, cloud, and recording." }} description={<Subtitle variant="changelog">April 2026</Subtitle>}>
  ## Scope: v0.2.2
  ...
  - **Multi source / multi sink** – Workflows can contain multiple source and sink nodes in a single graph. Each source routes to its own input track, each sink reports its own FPS and bitrate, and per-node recording captures each sink independently. Works in local, cloud, and headless modes.
  - **Audio In node** – A new graph node for audio inputs. WAV, MP3, FLAC, and OGG files can be uploaded to the asset library, picked from a new audio media picker, and forwarded into pipelines.
  - **Headless MPEG-TS streaming** – A new endpoint for streaming output in headless mode.
  - **Keyboard shortcuts** – Added keyboard shortcuts and a fullscreen button on the Sink node.

  #### Updates

  - **External parameter sync** – OSC, REST, and MCP parameter updates now sync to graph mode, with persistent permission grants and string primitive auto-send.
  - **LoRA controls in graph mode** – Added LoRA merge strategy view, integer handling, and prompt management.

  #### Bug Fixes

  - **Cloud connect** – Unified cloud connect flow and surfaced clearer connection errors.
  - **Workflow refresh** – Workflow Builder pipelines refresh after cloud connect.
