...
No user-facing changes were merged to [go-livepeer](https://github.com/livepeer/go-livepeer) this week. Recent work focused on internal review and in-progress branches; the last shipped change remains the trickle `/next` endpoint from May 13.
...
<Update label="May 13, 2026" tags={["Live AI", "Trickle"]} rss={{ title: "go-livepeer weekly update — May 13, 2026", description: "New trickle /next endpoint improves first-segment reliability for Live AI publishers" }} description={<Subtitle variant="changelog">May 13, 2026</Subtitle>}>
  ## Weekly update — May 13, 2026

### New features

- **Trickle `/<channel>/next` endpoint** - The trickle server now exposes a `GET /<channel>/next` route that returns the channel's next write sequence. Publisher clients can probe this endpoint before their first POST to align with the server's sequencing, which prevents the first segment from being dropped on fresh channels. This improves first-segment reliability for [Live AI](/v2/developers/concepts/ai-on-livepeer) pipelines that publish through trickle. The endpoint returns `404` if the channel does not exist and sets `Lp-Trickle-Closed: terminated` when the channel is closed.
