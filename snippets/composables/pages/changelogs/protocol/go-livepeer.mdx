...
<Update label="Unreleased" tags={["Live AI", "BYOC", "Livepeer Box", "Bug Fixes"]} rss={{ title: "go-livepeer unreleased", description: "Webcam input for Livepeer Box, BYOC worker auth tokens, Live AI payment tracking, and macOS build fix" }} description={<Subtitle variant="changelog">May 2026</Subtitle>}>
  ## Unreleased

### New features

- **Webcam input for Livepeer Box** - The Livepeer Box stream script now accepts a webcam (for example `/dev/video0`) as a video input source via v4l2, in addition to video files. This makes it easier to test Live AI pipelines end-to-end with a local camera. See the [box example gateway](https://github.com/livepeer/go-livepeer/blob/master/box/box.md) for usage.

### Updates

- **Auth tokens for BYOC worker registration** - BYOC orchestrators can now require an auth token when workers register, giving operators tighter control over which workers can join an orchestrator's BYOC capacity. See the [BYOC tutorial](/v2/gateways/guides/tutorials/tutorial-2-byoc-cpu-pipeline) for the BYOC architecture.
- **Live AI payment tracking events** - Live AI gateways now emit Kafka events when payments are sent to orchestrators, and increment a Prometheus counter on Kafka delivery errors. This improves payment observability for gateway operators running Live AI workloads.
