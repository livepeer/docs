...
Sets the error component that is shown when playback fails due to the stream being offline.
When this component is shown, the Player will retry playback in the background.
...
function PlayerComponent() {
  return (
    <Player
      ...
      streamOfflineErrorComponent={
        <div>
          Stream is offline. Playback will start automatically once the stream has started.
        </div>
      }
    />
  );
...
Sets the error component that is shown when playback fails due to an access control error.
This will override the
...
Sets the error component that is shown when playback fails for a generic reason (usually
this is an intermittent network issue).
When this component is shown, the Player will be retrying playback in the background,
with linear backoff. This will override the
