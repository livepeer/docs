...
<Warning>
  We're aware that some API requests through the SDKs are not working as
  expected. We're working on a fix and will update the documentation once it's
  resolved. In the meantime, please use the REST API directly.
</Warning>
