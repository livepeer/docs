...
First, create a livepeer `Client` instance using [`createReactClient`](/sdks/livepeer-js/client), and pass a provider to it.
...
`studioProvider` which is [CORS-protected API key](/sdks/livepeer-js/providers/studio#apikey).

[Read more about client configuration](/sdks/livepeer-js/client)

## Wrap app with `LivepeerConfig`

Next, wrap the app with the [`LivepeerConfig`](/sdks/livepeer-js/LivepeerConfig) component, passing the `client` to it.
