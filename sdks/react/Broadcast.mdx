...
description:
  ...
  `@livepeer/react/broadcast` components to create a full-featured broadcast
  interface with controls, settings, and device selection."
---
...
The broadcast interface consists of the following key components and features:

- `Broadcast.Root`: The root component that initializes and manages the
  broadcast context.
- `Broadcast.Container`: A container for the broadcast elements, ensuring proper
  styling and layout.
- `Broadcast.Video`: Displays the video stream.
- `Broadcast.LoadingIndicator`: Indicates the loading state with an animated
  icon.
- `Broadcast.ErrorIndicator`: Displays error messages and provides visual
  feedback when broadcasting fails.
- `Broadcast.Controls`: A set of interactive controls for managing the
  broadcast, including video and audio toggles, screenshare, picture-in-picture,
  and fullscreen.
  - `Settings`: A custom component to open a popover with stream settings.
    - `Broadcast.Select`: Allows selection of media input sources (camera and
      microphone) from a dropdown menu.
    - `RateSelectItem`: A custom component to display each item in the
      `Broadcast.Select` dropdown.
- `Broadcast.EnabledTrigger`: A button for starting and stopping the broadcast.
- `Broadcast.EnabledIndicator`: Provides visual cues for the broadcasting state
  (live or preview).
