 ...
 * @script            generate-seo
 * @category          generator
 * @purpose           seo:frontmatter-normalization
...
function resolveTargetFiles(repoRoot, args) {
  if (!args.file) {
    return collectAuthoredMdxFiles(repoRoot);
  }
...
function main() {
  ...
  const files = resolveTargetFiles(repoRoot, args);
  const results = {
    changed: 0,
    skipped: 0,
    errors: 0,
