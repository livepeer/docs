...
const { parseMdxFileWithFrontmatter } = require("../i18n/lib/frontmatter");
const {
  AUTHORED_MDX_ROOTS,
...
function hasLegacyOgValue(value) {
  const normalized = String(value || "").trim();
  const legacyAssetRoot = ["/snippets/assets", "domain", ""].join("/");
  const legacySharedLogo = ["LivepeerDocs", "Logo", ".svg"].join("");
  const legacySharedHero = ["LivepeerDocs", "Hero", ".svg"].join("");
  const legacySocialPreview = ["social", "preview", ""].join("-");
  if (!normalized) return false;
  return (
    normalized.includes(legacyAssetRoot) ||
    normalized.includes(legacySharedLogo) ||
    normalized.includes(legacySharedHero) ||
    normalized.includes(legacySocialPreview) ||
    /^https?:\/\//i.test(normalized)
  );
}

function isLegacySweepCandidate(repoPath, rawContent) {
  ...
  if (
    !AUTHORED_MDX_ROOTS.some(
      (rootPath) => normalized === rootPath || normalized.startsWith(`${rootPath}/`),
    )
  ) {
    return false;
  }
  ...
  try {
    ...
    return hasLegacyOgValue(data["og:image"] || data.ogImage);
  } catch (_error) {
    return false;
  }
}

function collectLegacyOgFiles(repoRoot, context) {
  const files = [];

  AUTHORED_MDX_ROOTS.forEach((rootPath) => {
    const rootAbsolutePath = path.join(repoRoot, rootPath);
    ...
    while (stack.length > 0) {
      ...
      entries.forEach((entry) => {
        ...
        const repoPath = toPosix(path.relative(repoRoot, fullPath));
        if (isAuthoredMdxPage(repoPath, context)) {
          return;
        }

        const rawContent = fs.readFileSync(fullPath, "utf8");
        if (isLegacySweepCandidate(repoPath, rawContent)) {
          files.push(fullPath);
        }
      });
    }
...
function resolveTargetFiles(repoRoot, context, args) {
  if (!args.file) {
    return [
      ...new Set([
        ...collectAuthoredMdxFiles(repoRoot, context),
        ...collectLegacyOgFiles(repoRoot, context),
      ]),
    ];
  }
  ...
  const eligible =
    isAuthoredMdxPage(repoPath, context) || isLegacySweepCandidate(repoPath, rawContent);
  if (!eligible) {
