...
function isAuthoredMdxPage(repoPath, context = null) {
  ...
  if (normalized.startsWith("v2/")) {
    ...
    return policyContext.routeMap.has(toSourceRouteKey(normalized));
  }
...
function resolveOgImageForFile(filePath, context = createOgImagePolicyContext()) {
  ...
  return {
    ...
    locale,
    sourceRoute,
    strict: generatedInternalReport || Boolean(tabDefinition),
    sectionId: generatedInternalReport ? "fallback" : tabDefinition?.id || "fallback",
    asset,
    fields: {
      "og:image": asset.path,
