...
const AUTHORED_MDX_ROOTS = ["v2", "docs-guide", "contribute", "snippets/pages"];
...
function getLocaleFromRepoPath(repoPath) {
  ...
  if (parts.length > 2 && SUPPORTED_LOCALES.includes(parts[1])) {
    return parts[1];
  }
  return "en";
}
...
function toSourceRouteKey(repoPath) {
  ...
  const parts = normalized.split("/");
  if (parts.length > 2 && SUPPORTED_LOCALES.includes(parts[1])) {
    return normalizeRouteKey(path.posix.join("v2", ...parts.slice(2)));
  }
  return normalized;
}
...
function buildEnglishTabDefinitions(repoRoot) {
  ...
  const en = Array.isArray(v2?.languages)
    ? v2.languages.find((languageNode) => languageNode?.language === "en")
    : null;
  const tabs = Array.isArray(en?.tabs) ? en.tabs : [];
...
function getLocalizedTabLabel(sectionId, locale) {
...
function buildManifestFromTabs(tabDefinitions) {
  ...
  tabDefinitions.forEach((tabDefinition) => {
    SUPPORTED_LOCALES.forEach((locale) => {
      const label = getLocalizedTabLabel(tabDefinition.id, locale);
      assets.push({
        kind: "section",
        locale,
