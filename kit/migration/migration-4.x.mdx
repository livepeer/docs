...
description: "Migration guide for the 4.0.0 release of Livepeer Kit."
---

<Info>
  Livepeer Kit is our newly released `@livepeer/react` experience, available in
  `v4.0.0+`. It was a complete rewrite of the library to support video
  primitives.
</Info>
...
- The React Native package has been deprecated. The package was not
  well-maintained and we have focused on web components for Livepeer Kit.
...
Check out our [complete example](/kit/Player) for how these can be composed to
build a video player, modeled after the 3.x versions of the Player.
...
Check out the [complete example](/kit/Broadcast) for how these can be composed
to build a complete in-browser broadcast experience.
...
2. You will now need to use the [`livepeer`](/sdks/javascript) JS SDK (or
   ...
   - You must now make API requests in your backend to fetch playback info for a
     playback ID.
   - Use [`getSrc`](/kit/player/get-src) utility function with the Player or
     [`getIngest`](/kit/broadcast/get-ingest) utility function with Broadcast to
