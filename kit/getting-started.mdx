...
description: "Get started building with Livepeer Kit."
---

<Info>
  Livepeer Kit is our newly released `@livepeer/react` experience,
  available in `v4.0.0+`.
</Info>
...
You can use `Player` primitives to build a media playback experience, and `Broadcast`
...
The player only includes a play/pause button, and the broadcast has a start/stop streaming
button.
...
Now that we've started using the primitives, we need something to play or broadcast! We can integrate with the
Livepeer Studio API to start playing media.

The below example shows the use of the `livepeer` JS SDK, but any other language's SDK can be used to fetch the
playback info or stream object.
