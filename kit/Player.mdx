...
title: "Player"
description:
  "A comprehensive example showcasing the integration of various
  `@livepeer/react/player` components to create a full-featured player interface
  with controls and settings."
---
...
The player interface consists of the following key components and features:

- `Player.Root`: The root component that initializes and manages the player
  context. It's responsible for setting up the player's environment and handling
  the provided source.
- `Player.Container`: A container for the player elements, ensuring proper
  styling and layout. It wraps all player-related components.
- `Player.Video`: Displays the video stream. It's the visual component where the
  actual video is rendered.
- `Player.LoadingIndicator`: Indicates the loading state with an animated icon.
  It provides visual feedback when the player is buffering or loading.
- `Player.ErrorIndicator`: Displays error messages and provides visual feedback
  for different types of player errors like offline, access-control, and other
  general errors.
- `Player.Controls`: A set of interactive controls for managing the player,
  including play/pause, mute/unmute, volume control, seek bar, fullscreen, and
  picture-in-picture.
  - `Player.PlayPauseTrigger`: A button for toggling play and pause states of
    the video.
  - `Player.LiveIndicator`: Indicates the live status of the stream and displays
    the current playback time.
  - `Player.MuteTrigger`: A button for toggling mute and unmute states of the
    video.
  - `Player.Volume`: A control for adjusting the volume of the video.
  - `Player.FullscreenTrigger`: A button for toggling fullscreen mode.
  - `Player.PictureInPictureTrigger`: A button for toggling picture-in-picture
    mode.
  - `Player.Seek`: A seek bar for navigating through the video.
  - `Player.Settings`: A custom component that provides a popover with playback
    settings, allowing users to adjust playback speed and quality.
    - `Player.RateSelect`: Allows selection of playback speed from a dropdown
      menu.
    - `Player.VideoQualitySelect`: Allows selection of video quality from a
      dropdown menu.
  - `Player.ClipTrigger`: A custom component for creating video clips from the
    current live stream.
