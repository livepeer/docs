...
- Ensure you are interacting with the correct contracts. Addresses can be found [here](/references/contract-addresses).
- **If you use a contract account rather than an EOA**: You will need to interact directly with the Migrator contract methods following the [Contract Wallet Migration](/orchestrators/guides/migrate-from-contract-wallet) guide.
