...
should use ICE servers. See the
[Stream via Browser](/guides/developing/stream-via-browser#adding-broadcasting-with-plain-webrtc).
docs for the exact steps to get the ICE config which should be used with playback - the steps are the exact same to get a config similar to:
