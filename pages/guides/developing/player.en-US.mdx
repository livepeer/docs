...
```
<Callout type="warning">
  When you make a request for playback URLs, in the response MP4 URLs are always listed 
  before HLS URLs. Additionally, each MP4 URL includes additional metadata about the video, 
  such as its width, height, bitrate, and size. This metadata can be useful for mobile 
  applications that want to optimize playback quality and size based on the viewer's 
