...
If it corresponds to a livestream, it will respond with:
...
<Callout>
  ...
  to use a video player component that supports these protocols. For
...
should use ICE servers. This is similar to the
[Stream via Browser](/guides/developing/stream-via-browser#adding-broadcasting-with-plain-webrtc)
docs for the steps to get the ICE config which should be used with playback.
...
To do this, make a GET request to the WebRTC URL which was returned from the playback info endpoint:
...
The server will respond with an SDP response that you can then use to set up your WebRTC playback!
