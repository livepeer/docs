...
<Callout type="warning" emoji="⚠️">
  **This example illustrates how Livepeer can be used as a video processing and
  ...
  IPFS/Arweave while the video is being uploaded to Livepeer, but do not recommend it for typical usage.
</Callout>

<Callout type="warning" emoji="⚠️">
  **For rapid processing of assets that will also be archived on IPFS or Arweave, we strongly encourage either (1) uploading to Livepeer with the [IPFS storage option enabled](/guides/developing/upload-a-video-asset#step-3-upload-video-to-ipfs), or (2) simply uploading the raw file to Livepeer via `useCreateAsset` or the API, rather than passing the IPFS / Arweave gateway URL. The gateway URL will work, but may incur longer-than-usual processing time.
</Callout>
