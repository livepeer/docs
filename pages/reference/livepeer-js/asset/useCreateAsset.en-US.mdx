...
The `storage` configs can be applied on asset creation. See `useUpdateAsset` for more details on these configs.

If the `storage` parameter is omitted, the asset will not be stored in decentralized storage.
