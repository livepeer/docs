...
The new **AI Subnet** is the first step in bringing powerful AI video
capabilities into the Livepeer network. It enables video developers to add
**generative AI features** such as `text-to-image`, `image-to-image` and
`image-to-video` conversions to their applications, and allows node operators to
**earn revenue by deploying their GPU resources** for AI processing tasks. For
an in-depth understanding of the AI Subnet and its current capabilities,
continue reading. Ready to dive in? Choose one of the cards below to kickstart
...
- **Decentralization**: Enhances security and resilience by eliminating single
  points of failure.
- **Cost-Effectiveness**: Promises to offer AI inference capabilities at much
  lower costs than conventional cloud services, time will tell.
...
**decentralized payment infrastructure** for efficient AI inferenace task
...
- **AI Gateway Nodes**: These nodes manage the flow of tasks, directing them to
  the appropriate Orchestrators based on capability and current load, ensuring
  efficient task allocation and system scalability.

The infrastructure is designed for **scalability**, enabling easy integration of
additional Orchestrators and Gateways as demand increases. It relies on a
...
- **Pre-loaded Models**: Models that are frequently used remain 'warm' on GPUs,
  speeding up task processing.
...
### Available Generative AI Pipelines

The AI Subnet supports various generative AI pipelines, primarily utilizing
[Diffusion models](https://huggingface.co/docs/diffusers/en/index). Although the
current focus is on Diffusion models, the aim is to expand support to other
model types in future updates. For more information on the available AI
pipelines, visit the [AI Pipelines](/ai/pipelines) page.
...
<Card
  ...
  iconType="duotone"
>
  Discover more about the available AI pipelines on the AI Subnet.
</Card>
...
- **Mainnet Transcoding Network**: Comprises Orchestrators and Gateways that
  perform or coordinate transcoding tasks.
- **AI Subnet**: A specialized subnet within the Livepeer network, designed for
  managing AI inference tasks.
...
- **Mainnet Transcoding Network Gateway**: A node that routes transcoding tasks
  to the appropriate Orchestrators. Often referred to as _Gateway_.
...
- **AI Gateway**: A specialized node that routes AI tasks to the appropriate AI
  Orchestrators on the AI Subnet.
