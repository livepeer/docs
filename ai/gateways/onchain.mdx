...
<Steps>
  ...
  <Step title="Set Max Price Per Pipeline and Model (Optional)">
    Set max price per unit for one or many pipeline/models by adding `-maxPricePerCapability /path/to/maxPrices.json` flag to AI Gateway launch.
    
    If a price is not set for a pipeline/model the `-maxPricePerUnit` price will be used.
    
    The flag input should be json file with the max prices specified.
    Example below will do the following:
    - set price for `image-to-image` pipeline and model `ByteDance/SDXL-Lightning` to 1700000 wei per unit. (image-to-image units is pixels)
    - set price for `text-to-image` pipeline and model `stabilityai/stable-diffusion-3-medium-diffusers` to 4768371 wei per unit (text-to-image units is pixels)
    - set price for `upscale` pipeline for all models to 4768371 wei per unit. `pixels_per_unit` defaults to `1` if not specified. (image-to-image units is pixels)
    - set price for `image-to-video` pipeline for all models to 3390842 wei per unit. (image-to-video units is pixels)
    - set price for `audio-to-text` pipeline for all models to 3390842 wei per unit. (audio-to-text units is milliseconds of audio)
    ```
    {
       "capabilities-prices":
      [
