...
The `audio-to-text` pipeline transcribes audio from media files to text. This pipeline powered is by the latest diffusion models at HuggingFace 
...
To generate an image with the `audio-to-text` pipeline, send a `POST` request to
the Gateway's `audio-to-text` API endpoint:
...
<Note>
Supported file types: `flac`, `mp3`, `m4a` and `webm`
</Note>
