...
AI model configuration file that specifies which AI pipelines and models to load.

### 3. models/ directory

Directory where AI model weights are stored.

## Optional Files

### 4. transcodingOptions.json (optional)

Video transcoding profile configuration if you want custom profiles beyond defaults.

### 5. password.txt (optional, on-chain only)

File containing your Ethereum keystore password for on-chain operations.
...
The gateway will handle both video transcoding (via RTMP ingest on port 1935) and AI processing (via HTTP API on port 8935) simultaneously.

## Notes

- The `aiModels.json` file defines which AI pipelines and models the gateway will support
...
- The dual gateway exposes both traditional video endpoints and AI processing endpoints through the same HTTP server
