...
>
 ...
 If you want to bring an existing wallet - put the public address in the `-ethAcctAddr` flag.
    ...
    ```bash icon="terminal" wrap lines Existing Wallet, New Keystore
    livepeer -gateway \
        -network arbitrum-one-mainnet \
...
<StyledStep title="RPC URL `-ethUrl` flag" titleSize="h2">
 ...
 These free-tier RPCs should be sufficient to run a single Gateway with the latest versions of Livepeer.
