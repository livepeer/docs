...
Without a keystore on the node, make the same two calls from your wallet with
[`cast`](https://getfoundry.sh/cast/overview). The contract takes fee share in millionths, so the
example values above become:

```bash
# BondingManager: transcoder(rewardCut, feeShare)
cast send 0x35Bcf3c30594191d53231E4FF333E8A770453e40 \
  "transcoder(uint256,uint256)" 100000 50000 \
  --rpc-url <ARBITRUM_RPC_URL> --keystore <ORCHESTRATOR_KEYSTORE>
# ServiceRegistry (use 0x04C0b249740175999E5BF5c9ac1dA92431EF34C5 if you serve AI)
cast send 0xC92d3A360b8f9e083bA64DE15d95Cf8180897431 \
  "setServiceURI(string)" "https://YOUR_PUBLIC_IP:8935" \
  --rpc-url <ARBITRUM_RPC_URL> --keystore <ORCHESTRATOR_KEYSTORE>
```

On a hardware wallet, replace `--keystore <ORCHESTRATOR_KEYSTORE>` with `--ledger` or `--trezor`.
From a Safe, make the same calls in the [Transaction Builder](https://app.safe.global) or
[Den](https://www.onchainden.com). If the UI cannot load a function, paste the output of
`cast calldata "transcoder(uint256,uint256)" 100000 50000` as custom hex data instead.
...
<Warning>
  **Treat this host as compromisable.** It exposes port 8935 to the internet. Isolate it (dedicated
  host or VM), firewall all other ports, keep `go-livepeer` updated, and store nothing on it beyond
  the throwaway key. If you activated from a keystore here, finish step 7, then move it to cold
  storage and delete it from the node.
</Warning>
...
<Tabs>
  ...
  <Tab title="Standalone caller">
    The wallet of a caller you run yourself on a separate host — either
    [livepeer-reward-caller](https://github.com/rickstaa/livepeer-reward-caller), a single static
    binary with its own keystore, or a second `go-livepeer` started with only
    `-reward -ethOrchAddr <ORCHESTRATOR_ADDR>` and that wallet's keystore. Start the node with
    `-reward=false`.
  </Tab>
...
`livepeer_cli` if the keystore is on the node (same hardware wallet and Safe swaps as step 4):
...
Confirm it took effect with a simulated reward call (no key or gas needed). A
`caller must be a reward caller set by the transcoder` revert means the address is wrong. Any other
result means the caller is set:

```bash
cast call --from <REWARD_CALLER_ADDR> 0x35Bcf3c30594191d53231E4FF333E8A770453e40 \
  "rewardForTranscoder(address)" <ORCHESTRATOR_ADDR> --rpc-url <ARBITRUM_RPC_URL>
```

Set it to `0x0000000000000000000000000000000000000000` to revoke. Arbiscan does not show this method
on the proxy page yet, but it is live on-chain. From a Safe, paste the output of `cast calldata`
as custom hex data.
