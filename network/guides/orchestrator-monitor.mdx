...
<Warning>
  Port 7935 is the node's private CLI port. Keep `-cliAddr` on loopback and scrape it locally or over
  a private network — never expose it to the internet.
</Warning>
...
<Warning>
  ...
  [Activate on Arbitrum](/network/guides/orchestrator-activate#8-confirm-reward-calling).
</Warning>
