...
- **~~P0 cron-is-dry-run bug.~~ FIXED 2026-05-25** in commit `e42946cdf` on `docs-v2-dev-draft` (pending merge to `docs-v2`). Inverted FLAGS default in all 6 `dispatch-{concern}.yml` scheduled jobs. Was the root cause of `llms.txt` + `sitemap-ai.xml` 47-day staleness, contracts data 19-day staleness, and "manual-only" classification of OG/SEO generators.
