...
| --- | --- | --- |
| **Ownerless governance** | A surface is not production-ready unless it has one canonical source, one validator, one repair path, and one primary gate | [`policies/ownerless-governance.mdx`](../policies/ownerless-governance.mdx) · `operations/governance/config/ownerless-governance-surfaces.json` (8 surfaces) + `repo-governance-surfaces.json` (28 surfaces) |
...
| 5 | **Adaptive architecture** | [Adaptive Architecture](./adaptive-architecture.mdx) | `operations/governance/config/`, `operations/scripts/remediators/`, `.githooks/`, `lpd repair` | 8 of 28 surfaces formally ownerless-ready; 5-stage control loop (detect/explain/repair/verify/record); honest framing |
...
Per the 2026-05-23 audit, **8 of 28 surfaces fully satisfy this contract** ([`ownerless-governance-surfaces.json`](https://github.com/livepeer/docs/blob/docs-v2-dev/operations/governance/config/ownerless-governance-surfaces.json)). The other 20 are in [`repo-governance-surfaces.json`](https://github.com/livepeer/docs/blob/docs-v2-dev/operations/governance/config/repo-governance-surfaces.json) at `advisory` state. The Gap Analysis is the queue for promoting each to `autofix` or `blocking`.
