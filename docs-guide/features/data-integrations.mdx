...
description: >-
  ...
  pipeline is gold-standard; OpenAPI is the weakest link. Verified 2026-05-23.
audience: developer
pageType: overview
purpose: orient
status: current
...
**The crisis on 2026-05-23:** **every Pattern A integrator that depends on cron writes is currently broken** because of the [cron-is-dry-run bug](./automations.mdx#cron-is-dry-run-by-default-bug-p0-systemic) in the dispatch workflows. The contracts pipeline ran today (`_health-checks.json` mtime 2026-05-22) but **the data files were last written 2026-05-04 – 19 days stale**. Same root cause for `llms.txt` + `sitemap-ai.xml` 47-day staleness. Fix one workflow file, unblock 6+ data families.
...
**BUT:** the data files in `snippets/data/contract-addresses/` were last written **2026-05-04** (19 days stale). The verification check ran today; the publish gate hasn't fired in 19 days. Root cause is the [cron-is-dry-run bug](./automations.mdx#cron-is-dry-run-by-default-bug-p0-systemic) – `dispatch-maintenance.yml` scheduled job runs with empty `FLAGS=""` because cron cannot pass `inputs.dry_run = false`, so the script runs in dry-run by default. **The post-merge job DOES write** (`--write` is hardcoded there), but it only fires on push to `docs-v2` touching `snippets/components/` or `snippets/data/` or `docs.json` – not on changes to the upstream protocol repos.
...
`_health-checks.json` runs daily; data files last written 2026-05-04. Root cause is the [cron-is-dry-run bug](./automations.mdx#cron-is-dry-run-by-default-bug-p0-systemic). **Fix:** invert the FLAGS pattern in `dispatch-maintenance.yml` scheduled job.
...
- **[Automations](./automations.mdx)** – the dispatch architecture + the cron-is-dry-run bug that breaks every cron-driven integrator
