...
description: >-
  ...
  adapters. 35 local skills, 53 portable templates, 6 native adapters, MCP +
  llms.txt + AI-enriched sitemap. Verified 2026-05-25.
audience: developer
pageType: reference
purpose: orient
status: current
...
The third strand – the **automation belt that keeps both honest** – was partially broken until 2026-05-25: `llms.txt` and `sitemap-ai.xml` had drifted 47 days stale, root-caused by the [cron-is-dry-run bug](./automations.mdx#cron-is-dry-run-by-default-bug-fixed-2026-05-25) in the dispatch workflows. Fix shipped 2026-05-25 in commit `e42946cdf` on `docs-v2-dev-draft`; the next scheduled cron after merge to `docs-v2` should regenerate both files. See [Known gaps](#known-gaps).
