...
| --- | --- | --- | --- |
| **P0** | **Cron-is-dry-run-by-default bug** – every `dispatch-{concern}.yml` scheduled job runs in dry-run mode because cron physically cannot pass `inputs.dry_run = false` | `dispatch-maintenance.yml` schedules daily 05:00 UTC; data files in `snippets/data/contract-addresses/` last written 2026-05-04 (19 days stale); same root cause for `llms.txt` + `sitemap-ai.xml` 47 days stale | Invert FLAGS pattern in all 6 dispatch workflows OR add separate scheduled-write job. **Single fix unblocks 6+ data pipelines.** See [Automations § Known gaps](./automations.mdx#known-gaps). |
...
| **P1** | `decisions-log.mdx` header dated 2026-03-31 but claims 18 decisions through D-GOV-08 (2026-05-22) | First 100 lines only show D-ACT-01..04; D-ACT-05..10 + all D-GOV-* entries not visible in first read | Verify entries 5-18 are in the log OR append them |
...
| **P1** | Contracts pipeline doc references empty `.github/scripts/` folder | `.github/scripts/fetch-contract-addresses.js` doesn't exist there; moved to `operations/scripts/integrators/maintenance/contracts/` | Update [contracts deep-dive in data-integrations.mdx](./data-integrations.mdx#contracts-pipeline-deep-dive) refs + retire orphan `contracts-pipeline.mdx` |
...
| Cron-is-dry-run pattern across 6 dispatchers | Cron cannot pass `dry_run: false`; scheduled run defaults to dry-run | Invert FLAGS: `FLAGS="--write --verify"; if [[ ... == "true" ]] then FLAGS=""` in `dispatch-{brand, copy, discoverability, governance, health, maintenance}.yml` | Validate by running one cron + confirming write occurs |
...
| Stale path: `.github/scripts/fetch-contract-addresses.js` | Folder empty; script moved | Update all references in pipeline docs + contracts-pipeline.mdx | Grep for the old path repo-wide |
| `decisions-log.mdx` header date | Says 2026-03-31; 18 decisions claimed through 2026-05-22 | Append D-ACT-05..10 + D-GOV-01..08 OR verify they're in the log past line 100 | Manual review |
...
| `.github/scripts/` empty | Contracts script moved to `operations/scripts/` | Delete the empty folder |
