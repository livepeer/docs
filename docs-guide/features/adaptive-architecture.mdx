...
description: >-
  ...
  repair, verify, record), four-part ownerless contract per surface, 8 of 28
  surfaces formally ownerless-ready, 49 validators + 37 remediators + 31
  generators + 25 audits. Verified 2026-05-23.
audience: developer
pageType: overview
purpose: orient
status: current
...
**The novelty is not validators or hooks individually** – most large repos have those. The novelty is the manifest-driven five-stage control loop and the explicit retirement of "human-only context" as a dependency. **8 surfaces are formally ownerless-ready today; 28 in the wider registry; 20 still advisory.**
...
## Surface inventory (live, 2026-05-23)

| Manifest | Path | Entries | Purpose |
| --- | --- | --- | --- |
| **Ownerless surfaces** | `operations/governance/config/ownerless-governance-surfaces.json` | **8** | Surfaces under the full four-part contract; `gate_layer` + `rollout_state` declared per entry |
| **Repo governance surfaces** | `operations/governance/config/repo-governance-surfaces.json` | **28** | Broader inventory; 8 formally ownerless-ready + 20 still advisory |
...
| --- | --- | --- |
| Ownerless surface manifest | `operations/governance/config/ownerless-governance-surfaces.json` | Machine-readable contract: validators, repair commands, gate layer, rollout state |
