...
**Target end state:** Approximately 10 dispatcher workflows. Workflows are thin dispatchers; all business logic lives in governed scripts under `operations/scripts/`. Every dispatcher has a local CLI equivalent. Every detection routes to a response.
...
- Locked decisions: `.github/workspace/decisions-log.mdx` (D-ACT-01 through D-ACT-10, D-GOV-01 through D-GOV-07)
...
- Superseded: `.github/workspace/design/governance/consolidated-architecture.md` (historical 47-target design; current target is the published framework above)
