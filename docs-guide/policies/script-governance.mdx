...
Every script in `tools/scripts/` is placed at a path following this structure:
...
<Steps>
  <Step title="Classify it first">
    ...
    Place the file at `tools/scripts/<type>/<concern>/<niche>/<script-name>.js`.
  </Step>
  <Step title="Write the JSDoc header">
    Copy the example header above. Fill all 11 tags. Do not leave placeholder values.
    Run `node tools/scripts/validators/governance/compliance/review-governance-repair-checklist.js --staged` to validate.
  </Step>
  ...
  <Step title="Add to the registry">
    Run `node tools/scripts/generators/governance/catalogs/generate-script-registry.js` to update `tools/config/script-registry.json`. The catalog is regenerated automatically from the registry.
  </Step>
</Steps>
...
| Registry generator | `tools/scripts/generators/governance/catalogs/generate-script-registry.js` |
