...
Single JSON at `docs-guide/config/component-registry.json`. Generated by pre-commit hook when files in `snippets/components/` are staged. Contains all 7 enforced governance fields per component plus derived fields (`file`, `importPath`). Script: `tools/scripts/generators/components/library/generate-component-registry.js`.
