...
Use the[ Component Registry](https://claude.ai/chat/docs-guide/component-registry.json) or the published[ Component Library](https://claude.ai/chat/v2/resources/documentation-guide/component-library.mdx) pages. Every component has a status badge, props table, and copy-paste example.
...
1. **Only makes sense on frame-mode or portal pages?** → `page-structure/`
1. **Accepts children and arranges them spatially?** → `layout/`
1. **Formats or renders content for the reader?** → `content/`
1. **Otherwise** → `primitives/`

### **Build a new component**

1. Create your `.jsx` file in the correct category folder.
1. Use arrow function syntax (`export const X = () => ...`).
1. Add the full JSDoc block (14 governance fields + `@param` per prop + `@example`).
...
1. `is`**/**`has`** prefix** for booleans (`isCompact`, `hasIcon`)
1. `on`** prefix** for handlers (`onClick`, `onToggle`)
1. **Defaults in destructuring** (`({ variant = 'default' }) => ...`)
1. `children`** for slot content, named props for data**
1. **Spread last** (`<div className={...} {...rest}>`)
1. **Required props: no default. Optional: always have default.**
...
1. Docs generation (manual/chained): registry + `@param` + `@example` + OpenRouter LLM → published MDX pages
1. LLM failure fallback: template-generated prose from metadata (deterministic)
