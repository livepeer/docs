...
| `tools/scripts/validators/content/check-content-freshness.js` | Flags v2 pages with lastVerified frontmatter older than 180 days or missing entirely. | `node tools/scripts/validators/content/check-content-freshness.js [--scope full\|changed] [--threshold &lt;days&gt;] [--json]` | docs |
...
| `tools/scripts/validators/content/check-duplicate-content.js` | Detects near-duplicate MDX pages using content hashing and similarity scoring. | `node tools/scripts/validators/content/check-duplicate-content.js [--threshold &lt;0-1&gt;] [--json]` | docs |
...
| `tools/scripts/validators/structure/check-page-local-mdx.js` | Detects MDX fragment files in views/ and groups/ subdirectories that are not imported by any parent page. | `node tools/scripts/validators/structure/check-page-local-mdx.js [--fix] [--json]` | docs |
| `tools/scripts/validators/structure/check-portal-pages.js` | Validates that pages with pageType: portal frontmatter follow the required portal structure. | `node tools/scripts/validators/structure/check-portal-pages.js [--fix] [--json]` | docs |
| `tools/scripts/validators/structure/check-stale-issues.js` | Flags open GitHub issues with no activity for 60+ days as stale, 90+ days as abandoned. | `node tools/scripts/validators/structure/check-stale-issues.js [--stale-days &lt;N&gt;] [--abandoned-days &lt;N&gt;] [--label] [--json]` | docs |
| `tools/scripts/validators/structure/validate-framework-integrity.js` | Validates the script governance framework matches actual repo state. Checks headers, classifications, pipelines, and requirements traceability. | `node tools/scripts/validators/structure/validate-framework-integrity.js [--fix-index] [--verbose] [--json] [--check-only]` | docs |
