...
<AccordionGroup>
  <Accordion title="Lazy-load Mermaid.js">
    The bundled `mermaid.min.js` is 2.45 MB and loads on every preview open, even for pages with no diagrams. The fix is to check for at least one `mermaid` segment in the parsed output before injecting the `<script>` tag. This would reduce load time on non-diagram pages to near-instant.

    **Location to change:** `lib/webview-template.js` — conditionally include `<script src="${mermaidUri}">` only when `hasMermaid` is passed as a param; `lib/component-map.js` — export a flag alongside `renderSegments`.
  </Accordion>
  ...
  <Accordion title="Custom component hot-reload">
    When a `.jsx` file in `snippets/components/` is saved, re-render any open preview panels that import it. Currently the preview only re-renders on changes to the active `.mdx` file — component source changes are invisible until the `.mdx` file is also touched.
  </Accordion>
</AccordionGroup>
