...
```
tools/scripts/config/paths.config.json
```
...
```
tools/scripts/config/product-social-config.json
```
...
| `tools/scripts/config/product-social-config.json` | No auto-trigger — must manually run social data fetch workflows via workflow dispatch |
