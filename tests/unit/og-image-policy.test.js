...
function findInternalHubRoute(context) {
  const internalTab =
    context.tabDefinitions.find((tabDefinition) => tabDefinition.id === "internal-hub") ||
    context.tabDefinitions.find((tabDefinition) =>
      tabDefinition.routes.some((route) => route.startsWith("v2/internal/")),
    ) ||
    null;
  ...
  const route =
    internalTab.routes.find((candidate) => candidate.startsWith("v2/internal/")) ||
    internalTab.routes[0] ||
    null;
