---
title: Introduction
description: "Explore APIs, guides, and examples to integrate with Livepeer."
icon: "hand-wave"
---

Livepeer is a video infrastructure network for live and on-demand streaming.
Designed to give developers the freedom to innovate, creators autonomy from
platforms, and viewers a choice in their experience.

<CardGroup cols={2}>
  <Card title="Quickstart" href="/developers/quick-start" icon="bolt-lightning">
    Get started with Livepeer in less than 5 minutes.
  </Card>
  <Card title="Core Concepts" href="/developers/core-concepts" icon="file">
    Learn about the core concepts of Livepeer.
  </Card>
  <Card title="Guides" href="/developers/guides/overview" icon="book">
    Learn how to integrate Livepeer into your app.
  </Card>
  <Card
    title="API References"
    href="/api-reference/overview/introduction"
    icon="code"
  >
    Explore the Livepeer Studio API and SDKs.
  </Card>
</CardGroup>

## Explore the Livepeer SDKs

<CardGroup cols={3}>
  <Card title="Livepeer Kit" icon="react" href="/kit/getting-started">
    Build with composable React primitives for both broadcasting and playback.
  </Card>
  <Card title="JavaScript" icon="js" href="/sdks/javascript">
    Seamlessly integrate video in your JavaScript app.
  </Card>
  <Card title="Ruby" icon="gem" href="/sdks/ruby">
    Build with Livepeer video playback in your Ruby app.
  </Card>
</CardGroup>

[View all SDKs](/sdks/introduction)
