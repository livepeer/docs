...
## Using Livepeer Kit Broadcast

The example below show to use the Livepeer Kit
[`Broadcast`](/kit/broadcast/Root) component to broadcast from the web.
...
Now that our providers are set up, we use the [`Broadcast`](/kit/broadcast/Root)
with a stream key, from a stream we created. We also override some of the custom
styling to match the flow of our app!
