...
We demonstrate below how to [broadcast from the web](/developers/core-concepts/studio/in-browser-broadcast) using Livepeer's low latency WebRTC broadcast.
Developers can either use the Livepeer.js Broadcast React component, or build their own WebRTC solution.

## Using Livepeer.js Broadcast

The example below show to use the Livepeer.js [`Broadcast`](/sdks/livepeer-js/Broadcast) component to broadcast from the web.

### Configuring providers

First, we [create a new React client and wrap the app with `LivepeerConfig`](/sdks/livepeer-js/getting-started).

### Broadcast

Now that our providers are set up, we use the [`Broadcast`](/sdks/livepeer-js/Broadcast) with a stream key, from a stream we created.
...
If you want to add custom broadcasting to your app and handle the WebRTC SDP negotiation, you can follow the steps below.
