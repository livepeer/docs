...
the docs [here](/developers/guides/get-engagement-analytics-via-api).
...
1. Integrated the necessary components to capture viewership data. Detail
   [here](/developers/guides/get-engagement-analytics-via-api#registering-views).
2. Viewed videos via (1) to collect data.
3. Created an [CORS-enabled API Key](/api-reference/overview/authentication).
   The key must allow CORS access from your Grafana origin, or from all (\*)
4. Set up a Grafana account and workspace. A free account is sufficient and
   available at [Grafana.com](https://grafana.com/)
...
3. Install “JSON Api” by either searching for it or using this url:
   `<your-grafana-url>/grafana/connections/datasources/marcusolsson-json-datasource`
4. Set up your JSON Api:
   1. Name: e.g. “Engagement Data”
   2. URL:
      [`https://livepeer.studio/api/data/views/query`](https://livepeer.studio/api/data/views/query)
   3. Custom HTTP Headers
      1. Header: “Authorization”
      2. Value: `Bearer <your api key>`
5. Save and Test
6. Get your data source’s `uid` from the browser's url, e.g.
   `<your-grafana-url>/grafana/connections/your-connections/datasources/edit/<datasource-uid>`
7. Copy and paste this uid into the json template
   1. json template available:
      [here](https://gist.github.com/ecmulli/20d7f24c784ba17eb53278c3014ac638)
