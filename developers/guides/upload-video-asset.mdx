...
<Tabs>
  ...
  <Tab title="Python">
    ...
    lpClient = livepeer.SDK( api_key="" )

    req = components.NewAssetPayload(name='filename.mp4')

    res = lpClient.asset.create(req)

    if res.data is not None: # handle response pass
      ```
  ...
  <Tab title="Ruby">
    ```ruby
      ...
      res = lpClient.asset.create(request=Shared::NewAssetPayload.new(
          name="filename.mp4",
        )
      )
  ...
  <Tab title="Go">
    ...
    func main() {
        ...
        res, err := lpClient.Asset.Create(ctx, components.NewAssetPayload{
            Name: "filename.mp4",
        })
