...
either Livepeer.js or Video.js.
...
The example below show to use the Livepeer.js
[`Player`](/sdks/livepeer-js/Player) to view a video asset, with some custom
...
We
[create a new livepeer.js client and wrap the app with `LivepeerConfig`](/sdks/livepeer-js/getting-started).

### Play Video

Now that our providers are set up, we use the
[`Player`](/sdks/livepeer-js/Player) with an IPFS CID as our `playbackId`, which
we created previously when uploading a video asset and
[storing to IPFS](/sdks/livepeer-js/asset/useUpdateAsset). We use
...
Using livepeer.js is the recommended way to play back a video or a livestream -
...
[Player's technical details](/sdks/livepeer-js/Player#technical-details) for
more information on how livepeer.js handles this.
