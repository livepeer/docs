...
either Livepeer Kit or Video.js.
...
## Using the Livepeer Kit Player

The example below show to use the Livepeer Kit [`Player`](/kit/player/Root) to
view a video asset, with some custom styles to demonstrate what's possible.
...
We
[create a new React client and wrap the app with `LivepeerConfig`](/kit/getting-started).

### Play Video

Now that our providers are set up, we use the [`Player`](/kit/player/Root) with
an IPFS CID as our `playbackId`, which we created previously when uploading a
video asset and storing to IPFS. We use
...
Using Livepeer Kit is the recommended way to play back a video or a livestream -
it handles MP4 renditions, errors from the API, and is a true web3-native media
player. However, if you want to use an alternative, you can do so by following
...
[Player's technical details](/kit/player/Root#technical-details) for more
...
<Info>
  ...
  Livepeer Kit Player automatically handles this.** See the [Player
  docs](/kit/player/Root#technical-details) for more information.
</Info>
