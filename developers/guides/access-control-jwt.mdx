...
If you are not using the player, you will need to manually append the jwt to the
URL as a query parameter, similar to:

```
https://playback.livepeer.studio/asset/hls/abcd1234/index.m3u8?jwt=your-jwt
```

If you prefer not to use query parameters to pass the JWT, 
it can alternatively be transmitted as a header. To accomplish this, 
you must set a header named `Livepeer-JWT` with your access key as its value.

Below is an example of how to use headers when using 
the Livepeer Player:
...
const hlsConfig = {
  xhrSetup: function(xhr, url) {
    xhr.setRequestHeader('Livepeer-Jwt', 'YOUR_JWT');
  }
};
