---
title: "Stream with ACL"
description: "Learn how to add access control to a stream with livepeer.js"
---

Adding access control to a stream only takes a few lines of code. The example below uses [`useCreateStream`](/reference/livepeer-js/stream/useCreateStream)
and [`useStream`](/reference/livepeer-js/stream/useStream) to create and watch a gated livestream.
...
First step, we create a new livepeer.js client with the Studio provider and a [CORS-protected API key](/reference/livepeer-js/providers/studio#apikey):
...
Then, we pass the JWT to the Player using the [`jwt`](/reference/livepeer-js/Player#jwt) prop, which will use that JWT to prove access to the streaming content!
