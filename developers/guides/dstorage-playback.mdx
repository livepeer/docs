...
The raw input is passed to the [`Player`](/kit/player/Root), which automatically
detects if it is a valid CID and attempts to fetch the playback info for that
CID. If the provider does not have an `Asset` with that CID, the Player will
automatically attempt to upload the CID from IPFS, and then play the transcoded
content back, without any additional code on the frontend. To ensure that there
is no delay in playback, we recommend uploading via `useCreateAsset` before
