...
Webhook payloads contain information about the event that triggered the webhook. The payload is a JSON object that contains different properties depending on the event type. Here is an example of how the webhook playload looks like:
...
<ParamField path="event_object" type="string">
  The object payload such as stream/asset/task (More info about each payload can
  be found in the [API references](/api-reference/overview/introduction))
</ParamField>
