...
[`Player`](/sdks/react/player/Root) with a `playbackId`, which we created
...
Using Livepeer UI Kit is the recommended way to play back a video or a
...
<Warning>
  ...
  that handles choosing the correct MP4 rendition, gracefully recovers from
  routine RTMP reconnects through custom polling logic, and handles WHEP
  playback. In the absence of this capability, reconnects may severely degrade
  viewer experience and require a hard refresh to resolve, and an incorrect MP4
...
Streams also have a WHEP WebRTC low latency URL. Short-form assets will have an
MP4 source URL.
