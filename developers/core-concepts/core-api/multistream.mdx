...
When setting up a multistream, users have the option to utilize the Studio
dashboard, directly interface with Livepeer REST API, or the Livepeer SDK. These
offer comprehensive management tools to configure, modify, or delete specific
multistream targets linked with individual streams. It's important to note that
configurations are stream-specific; hence, they don't automatically transfer
when generating new streams.
