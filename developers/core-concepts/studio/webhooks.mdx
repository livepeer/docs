A webhook is a common way for developers to integrate real-time updates in their application by providing an endpoint which Livepeer Studio can send [events](/api-reference/webhook/create) to.
